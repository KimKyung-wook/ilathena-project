...
{
	if (id < 0)
		ShowNotice("모든 사용자 오프라인 전송\n");
	else
		ShowNotice("맵 서버 %d 사용자 오프라인 전송\n",id);
...
// Char structure of a version 1 (original data structure)
	...
	for(i = 0; str[next] && str[next] != '\t' && str[next] != '\n' && str[next] != '\r'; i++) { // global_reg렳몧댥멟궻athena.txt뚚듂궻궫귕덇돒'\n'?긃긞긏
...
//-----------------------------------
// iLAthena 수정 [한글사랑모드]
int make_new_char(struct char_session_data* sd, char* name_, unsigned char* dat, int str, int agi, int vit, int int_, int dex, int luk, int slot, int hair_color, int hair_style)
{
	...
	// iLAthena 수정 [한글사랑모드]
		...
		for( i = 0; i < NAME_LENGTH && name[i]; i++ )
			if( strchr(char_name_letters, name[i]) != NULL )
				return -2;
	// iLAthena 수정시작 [한글사랑모드]
	} else if (char_name_option == 3) {
		for (i = 0; i < NAME_LENGTH && dat[i]; i++) {
			if(dat[i] >= 0xAC && dat[i] <= 0xD7) {
				i++;
			} else if(!(dat[i] >= 'A' && dat[i] <= 'Z') || !(dat[i] >= 'a' && dat[i] <= 'z')) {
				return -2;
			}
		}
	// iLAthena 수정종료 [한글사랑모드]
	...
	ShowInfo("캐릭터 생성: 계정: %d, 케릭터: %d, 슬롯: %d, 이름: %s\n", sd->account_id, i, slot, name);
	mmo_char_sync();
	return i;
}
...
int parse_fromlogin(int fd)
{
	...
	if(session[fd]->flag.eof) {
		if (fd == login_fd) {
			ShowWarning("로그인 서버와의 연결이 끊어졌습니다. (연결 #%d).\n", fd);
			login_fd = -1;
		}
		do_close(fd);
		return 0;
	...
	while(RFIFOREST(fd) >= 2)
	{
		...
		case 0x2711:
			...
			if (RFIFOB(fd,2)) {
		ShowWarning(CL_BG_RED""CL_BT_WHITE"현재 서버계정 미설정 상태로 해킹위험에 노출됨\n"CL_RESET);
		ShowWarning(CL_BG_RED""CL_BT_WHITE"map_athena.conf char_athena.conf account.txt 서 s1/p1 변경권고 (성별 S)\n"CL_RESET);
			} else {
				ShowStatus("로그인 서버 연결 (연결 #%d).\n", fd);
...
//			WBUFW(buf,0) = 0x2b11;
		...
		default:
			ShowError("알 수 없는 패킷 0x%04x 을 로그인 서버로부터 받았습니다, 연결 해제.\n", RFIFOW(fd,0));
			set_eof(fd);
			return 0;
		}
	}
...
int parse_frommap(int fd)
{
	...
	while(RFIFOREST(fd) >= 2)
	{
		...
		case 0x2afa: // Receiving map names list from the map-server
			...
			ShowStatus("맵 서버 %d 연결: %d 개의 맵, IP %d.%d.%d.%d 포트 %d.\n",
						id, j, CONVIP(server[id].ip), server[id].port);
			ShowStatus("맵 서버 %d 읽음\n", id);
			char_log("맵 서버 %d 연결: %d 개의 맵s, IP %d.%d.%d.%d 포트 %d. 맵 서버 %d 읽기 완료.\n",
...
//					result = 2; // 2-gm level too low
		...
		case 0x2736: // ip address update
			...
			ShowInfo("%d.%d.%d.%d 로 부터 맵 서버 #%d 의 IP주소를 갱신했습니다.\n", id, CONVIP(server[id].ip));
			RFIFOSKIP(fd,6);
		break;
		...
		default:
		{
			...
			ShowError("알 수 없는 패킷 0x%04x 을 맵 서버로부터 받았습니다, 연결 해제.\n", RFIFOW(fd,0));
			set_eof(fd);
			return 0;
		}
		} // switch
...
// char_mapif궻룊딖돸룉뿚걁뙸띪궼inter_mapif룊딖돸궻귒걂
...
int lan_subnetcheck(uint32 ip)
{
	...
	if ( i < subnet_count ) {
		ShowInfo("보조네트워크검사 [%u.%u.%u.%u]: Matches "CL_CYAN"%u.%u.%u.%u/%u.%u.%u.%u"CL_RESET"\n", CONVIP(ip), CONVIP(subnet[i].char_ip & subnet[i].mask), CONVIP(subnet[i].mask));
		return subnet[i].char_ip;
	} else {
		ShowInfo("보조네트워크검사 [%u.%u.%u.%u]: "CL_CYAN"WAN"CL_RESET"\n", CONVIP(ip));
		return 0;
	}
}
...
int parse_char(int fd)
{
	...
	while( RFIFOREST(fd) >= 2 )
	{
				...
				return 0;
		{
			...
			ShowInfo("연결요청 - 계정ID:%d/로그인ID1:%d/로그인ID2:%d\n", account_id, login_id1, login_id2);
		...
		case 0x67:
			...
			if( !char_new ) //turn character creation on/off [Kevin]
				...
				// iLAthena 수정 [한글사랑모드]
				i = make_new_char(sd, (char*)RFIFOP(fd,2),RFIFOP(fd,2),RFIFOB(fd,26),RFIFOB(fd,27),RFIFOB(fd,28),RFIFOB(fd,29),RFIFOB(fd,30),RFIFOB(fd,31),RFIFOB(fd,32),RFIFOW(fd,33),RFIFOW(fd,35));
			...
			if (cmd == 0x1fb) FIFOSD_CHECK(56);
		{
			...
			ShowInfo(CL_RED"캐릭터 삭제요청: "CL_GREEN"%d (%d)"CL_RESET"\n", sd->account_id, cid);
...
// 멣궲궻MAP긖?긫?궸긢??몭륪걁몭륪궢궫map랪궻릶귩뺅궥걂
...
// 렔빁댥둖궻멣궲궻MAP긖?긫?궸긢??몭륪걁몭륪궢궫map랪궻릶귩뺅궥걂
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if(strcmpi(w1,"timestamp_format") == 0) {
			...
			if (bind_ip) {
				...
				ShowStatus("캐릭터 서버 binding IP 주소 : %s -> %s\n", w2, ip2str(bind_ip, ip_str));
			}
...
int do_init(int argc, char **argv)
{
	...
	ShowInfo("캐릭터 서버 설정완료\n");

	// a newline in the log...
	char_log("");
	char_log("The char-server starting...\n");

	ShowInfo("캐릭터 서버 최적화 시작\n");
	...
	if ((naddr_ != 0) && (!login_ip || !char_ip))
	{
		...
		if (naddr_ > 1)
			ShowStatus("여러개의 네트워크 검색. %s 의 IP주소 사용\n", ip_str);
		else
			ShowStatus("%s IP주소로 기본 설정\n", ip_str);
		if (!login_ip) {
	...
	ShowStatus(CL_BG_GREEN""CL_WHITE"캐릭터 서버 실행 완료\n"CL_NORMAL);
