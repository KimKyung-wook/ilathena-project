...
int parse_fromchar(int fd)
{
	...
	{
		ShowStatus(CL_BG_RED""CL_BT_WHITE"캐릭터 서버 '%s' 와의 연결 끊어짐\n"CL_RESET, server[id].name);
...
int mmo_auth(struct login_session_data* sd)
{
	...
	if( acc.unban_time != 0 && acc.unban_time > time(NULL) )
	{
		...
		ShowNotice("연결거절 (계정: %s, 암호: %s, 차단: %s, IP: %s)\n", sd->userid, sd->passwd, tmpstr, ip);
		return 6; // 6 = Your are Prohibited to log in until %s
	}

	if( acc.state != 0 )
	{
		ShowNotice("연결거절 (계정: %s, 암호: %s, 상태: %d, IP: %s)\n", sd->userid, sd->passwd, acc.state, ip);
		return acc.state - 1;
	}

	ShowNotice("인증 완료 (계정: %s, id: %d, ip: %s)\n", sd->userid, acc.account_id, ip);
...
void login_auth_ok(struct login_session_data* sd)
{
	...
	if( sd->level > 0 )
		ShowStatus("GM계정 (GM레벨:%d) '%s' 접속 완료.\n", sd->level, sd->userid);
	else
		ShowStatus("일반계정 '%s' 계정 접속 완료.\n", sd->userid);
...
int parse_login(int fd)
{
	...
	if( session[fd]->flag.eof )
	{
		ShowInfo("'"CL_WHITE"%s"CL_RESET"' 연결됨.\n", ip);
		do_close(fd);
		return 0;
	}
	...
	while( RFIFOREST(fd) >= 2 )
	{
				...
				return 0;
		{
			...
			ShowInfo("'%s' 서버 연결 시도중 @ %u.%u.%u.%u:%u (계정: '%s', 암호: '%s', IP: '%s')\n", server_name, CONVIP(server_ip), server_port, sd->userid, sd->passwd, ip);
			...
			{
				ShowStatus(CL_BG_GREEN""CL_BT_WHITE"'%s' 서버연결 완료\n"CL_NORMAL, server_name);
		...
		default:
			ShowNotice("비정상적인 연결종료 (IP: %s): 0x%x 패킷오류\n", ip, command);
			set_eof(fd);
			return 0;
		}
	}
...
int do_init(int argc, char** argv)
{
	...
	ShowStatus(CL_BG_GREEN""CL_WHITE"로그인 서버 실행 완료\n"CL_NORMAL);
