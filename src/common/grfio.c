...
static void grfio_resourcecheck(void)
{
	...
	if (fp) {
		...
		fclose(fp);
		ShowStatus("'"CL_WHITE"resnametable.txt"CL_RESET"' 에서 '"CL_WHITE"%d"CL_RESET"' 개의 자료를 읽었습니다.\n", i);
	...
	if (buf) {
		...
		aFree(buf);
		ShowStatus("'"CL_WHITE"data\\resnametable.txt"CL_RESET"' 에서 '"CL_WHITE"%d"CL_RESET"' 개의 자료를 읽었습니다.\n", i);
		return;
	}
...
void grfio_init(char* fname)
{
	...
	if (data_conf) {
		...
		fclose(data_conf);
		ShowStatus("'"CL_WHITE"%s"CL_RESET"' 파일을 읽었습니다.\n", fname);
