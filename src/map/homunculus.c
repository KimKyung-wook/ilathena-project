...
int read_homunculusdb(void)
{
	...
	for(i = 0; i<2; i++)
	{
		...
		fclose(fp);
		ShowStatus("'"CL_WHITE"db/%s"CL_RESET"' 에서 '"CL_WHITE"%d"CL_RESET"' 개의 호문클루스를 읽었습니다.\n", filename[i], j);
	}
	return 0;
}
...
int read_homunculus_skilldb(void)
{
	...
	fclose(fp);
	ShowStatus("'"CL_WHITE"homun_skill_tree.txt"CL_RESET"' 파일을 읽었습니다.\n");
	return 0;
}
...
void read_homunculus_expdb(void)
{
	...
	for(i=0; i<2; i++){
		...
		fclose(fp);
		ShowStatus("'"CL_WHITE"db/%s"CL_RESET"' 에서 '"CL_WHITE"%d"CL_RESET"' 개의 레벨을 읽었습니다.\n", filename[i], j);
	}
}
