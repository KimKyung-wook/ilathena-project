...
extern int char_fd;		// inter server궻fd궼char_fd귩럊궎
#define inter_fd char_fd	// 긄귽깏귺긚
...
int intif_wis_message(struct map_session_data *sd, char *nick, char *mes, int mes_len)
{
	...
	if (battle_config.etc_log)
		ShowInfo("귓속말 %s 으로부터 %s (내용: '%s')\n", sd->status.name, nick, mes);

	return 0;
}

// iLAthena 수정
int intif_wis_messageex(char *Wisp_name, char *nick, char *mes, int mes_len)
{
...
//-----------------------------------------------------------------
// inter server궔귞궻믅륪
// 긄깋?궕궇귢궽0(false)귩뺅궥궞궴
// 긬긑긞긣궕룉뿚궳궖귢궽1,긬긑긞긣뮮궕뫉귟궶궚귢궽2귩뺅궥궞궴
