...
int mapif_parse_CreateParty(int fd, char *name, int item, int item2, struct party_member *leader)
{
	...
	// iLAthena 수정시작 [한글사랑모드]
		...
		if (char_name_option == 3) {
			for (j = 0; j < NAME_LENGTH && dat[j]; j++) {
				if(dat[j] >= 0xAC && dat[j] <= 0xD7) {
					j++;
				} else if(!(dat[j] >= 'A' && dat[j] <= 'Z') || !(dat[j] >= 'a' && dat[j] <= 'z')) {
					mapif_party_created(fd,leader->account_id,leader->char_id,NULL);
					return 0;
				}
			}
...
// map server 궔귞궻믅륪
// 갋괦긬긑긞긣궻귒됶먏궥귡궞궴
// 갋긬긑긞긣뮮긢??궼inter.c궸긜긞긣궢궲궓궘궞궴
// 갋긬긑긞긣뮮?긃긞긏귘갂RFIFOSKIP궼뚁귂뢯궢뙰궳뛱귦귢귡궻궳뛱궯궲궼궶귞궶궋
// 갋긄깋?궶귞0(false)갂궩궎궳궶궋궶귞1(true)귩궔궑궠궶궚귢궽궶귞궶궋
