...
#define PVP_CALCRANK_INTERVAL 1000	// PVP룈댧똶럁궻듩둼
...
bool pc_authok(struct map_session_data *sd, int login_id2, time_t expiration_time, int gmlevel, struct mmo_charstatus *st)
{
	...
	if( sd->gmlevel >= battle_config.lowest_gm_level )
		// iLAthena 수정
		ShowInfo("운영자 '"CL_WHITE"%s"CL_RESET"' 로그인."
			" (계정ID/캐릭터ID: '"CL_WHITE"%d/%d"CL_RESET"',"
			" 패킷버전: '"CL_WHITE"%d"CL_RESET"', IP: '"CL_WHITE"%d.%d.%d.%d"CL_RESET"',"
			" GM레벨 '"CL_WHITE"%d"CL_RESET"').\n",
			sd->status.name, sd->status.account_id, sd->status.char_id,
			sd->packet_ver, CONVIP(ip), sd->gmlevel);
	else
		// iLAthena 수정
		ShowInfo("'"CL_WHITE"%s"CL_RESET"' 로그인."
			" (계정ID/캐릭터ID: '"CL_WHITE"%d/%d"CL_RESET"',"
			" 패킷버전: '"CL_WHITE"%d"CL_RESET"', IP: '"CL_WHITE"%d.%d.%d.%d"CL_RESET"').\n",
			sd->status.name, sd->status.account_id, sd->status.char_id,
	...
	if (battle_config.display_version == 1){
		char buf[256];
		// iLAthena 수정
		sprintf(buf, "iLAthena SVN version: %s", get_svn_revision());
...
int pc_bonus(struct map_session_data *sd,int type,int val)
{
	...
	case SP_ADD_HEAL2_RATE:
		...
		break;
	default:
		ShowWarning("캐릭터 보너스 형식오류 (형식:%d 값:%d)\n",type,val);
		break;
	}
	return 0;
}
...
int pc_insert_card(struct map_session_data* sd, int idx_card, int idx_equip)
{
	...
	int nameid;
	// iLAthena 수정
	struct item_data *cdata;	// 카드확률조합시스템[★orz。]
	int percent;	// 카드확률조합시스템[★orz。]
	...
	{
		cdata=itemdb_search(nameid);

		if (cdata->probcard_percentage > 0) {
			percent=rand()%10000;

			if(percent > cdata->probcard_percentage)
			{
				clif_insert_card(sd,idx_equip,idx_card,1);	// 1.02 버전에서 수정
				if (cdata->probcard_del == 1)
				{
					pc_delitem(sd,idx_equip,1,0);
					clif_displaymessage(sd->fd, "장비가 파괴되었습니다."); 	// 1.02 버전에서 수정
				}
				else if (cdata->probcard_del == 2)
				{
					pc_delitem(sd,idx_card,1,0);
					clif_displaymessage(sd->fd, "카드가 파괴되었습니다."); 	// 1.02 버전에서 수정
				}
				else if (cdata->probcard_del == 3)
				{
					pc_delitem(sd,idx_card,1,0);
					pc_delitem(sd,idx_equip,1,0);
					clif_displaymessage(sd->fd, "장비와 카드가 파괴되었습니다."); 	// 1.02 버전에서 수정
				}
				return 0;
			}
		}	// 카드확률조합시스템[★orz。] ; 추가종료
...
int pc_modifybuyvalue(struct map_session_data *sd,int orig_value)
{
	...
	if((skill=pc_checkskill(sd,MC_DISCOUNT))>0)	// 긢귻긚긇긂깛긣
		rate1 = 5+skill*2-((skill==10)? 1:0);
	if((skill=pc_checkskill(sd,RG_COMPULSION))>0)	// 긓?긬깑긘깈깛긢귻긚긇긂깛긣
		rate2 = 5+skill*4;
...
int pc_modifysellvalue(struct map_session_data *sd,int orig_value)
{
	...
	if((skill=pc_checkskill(sd,MC_OVERCHARGE))>0)	// 긆?긫??긿?긙
...
int pc_additem(struct map_session_data *sd,struct item *item_data,int amount)
{
	...
	//차암 시스템 [페퍼민트]
	if(battle_config.keepitem_use && data->ki.exist)
		status_calc_pc(sd,0);
...
{
	short nameid = sd->status.inventory[n].nameid;	// 차암 시스템 [페퍼민트]
	...
	// 차암 시스템 [페퍼민트]
	if(battle_config.keepitem_use && itemdb_search(nameid)->ki.exist)
		status_calc_pc(sd,0);
...
{
	struct item_data *data = itemdb_search(sd->status.inventory[n].nameid); // iLAthena 수정
	nullpo_retr(1, sd);
	...
	if (battle_config.keepitem_use && data->ki.del)	// 차암 시스템 [페퍼민트]
		clif_displaymessage(sd->fd, "차암의 마법으로 드랍된 아이템은 사라졌습니다.");
	else if(data->imd.flag == 2 || data->imd.flag == 3)		// 즉시효과아이템 [★orz。]
	{
		int i;
		for(i=0; i<amount; i++)
...
int pc_takeitem(struct map_session_data *sd,struct flooritem_data *fitem)
{
	...
	struct item_data *data=itemdb_search(fitem->item_data.nameid); // iLAthena 수정
	...
	// 즉시효과아이템 [★orz。]
	if(data->imd.flag == 1 || data->imd.flag == 3)
	{
		int i;
		for (i=0; i<fitem->item_data.amount; i++)
			run_script(data->imd.script, 0, sd->bl.id, 0);
		clif_displaymessage(sd->fd, "변화가 생긴것 같다!");
	}
...
int pc_isUseitem(struct map_session_data *sd,int n)
{
	...
	if( !item->script ) //if it has no script, you can't really consume it!
		return 0;
	if(sd->state.blockeduseitem!=0) // @락다운 명령어 [JaVb]
		return 0;
...
int pc_setpos(struct map_session_data* sd, unsigned short mapindex, int x, int y, uint8 clrtype)
{
	...
	// 차암 시스템 [페퍼민트]
	if(battle_config.keepitem_use && sd->bl.m != m && clrtype > 0 && ((map[sd->bl.m].flag.pvp != map[m].flag.pvp) || (map[sd->bl.m].flag.gvg != map[m].flag.gvg)))
		status_calc_pc(sd,0);
...
int pc_randomwarp(struct map_session_data *sd, int type)
{
	...
	if (map[sd->bl.m].flag.noteleport)	// 긡깒??긣뗕?
		return 0;
 ...
 * 븧딇?뛛궸귝귡긚긌깑궻???긃긞긏
 * 덙?갌
 *   struct map_session_data *sd	긜긞긘깈깛긢??
 *   int nameid						?뷈뷼ID
 * 뺅귟뭠갌
 *   0		?뛛궶궢
 *   -1		긚긌깑귩됶룣
...
int pc_gainexp(struct map_session_data *sd, struct block_list *src, unsigned int base_exp,unsigned int job_exp,bool quest)
{
	...
	// @이벤트 명령어 [JaVb]
	if(event_eflag==1) {
		base_exp=(base_exp+(base_exp/10*event_rate));
		job_exp=(job_exp+(job_exp/10*event_rate));
	}
...
int pc_jobchange(struct map_session_data *sd,int job, int upper)
{
	...
	for(i=0;i<EQI_MAX;i++) {
		if(sd->equip_index[i] >= 0)
			if(!pc_isequip(sd,sd->equip_index[i]))
				pc_unequipitem(sd,sd->equip_index[i],2);	// ?뷈둖궢
	}
...
int pc_setfalcon(TBL_PC* sd, int flag)
{
	...
	{ // You cannot get falcon while riding warg or while you have warg.
		if( pc_checkskill(sd,HT_FALCON)>0 && !(sd->sc.option&OPTION_RIDING_WUG || sd->sc.option&OPTION_WUG) )	// 긲?깑긓깛?긚?깏?긚긌깑룋렃
...
/*==========================================
 * sd궕dstsd궴뙅뜢(dstsd겏sd궻뙅뜢?뿚귖벏럃?s궎)
 *------------------------------------------*/
...
//			ShowDebug("%s - Class %d: %u\n", type?"Job":"Base", job_id, max_level[job][type]);
	...
	ShowStatus("'"CL_WHITE"exp.txt"CL_RESET"' 에서 경험치 정보를 읽었습니다.\n");
	...
	fclose(fp);
	ShowStatus("'"CL_WHITE"skill_tree.txt"CL_RESET"' 에서 스킬트리를 읽었습니다.\n");
	...
	fclose(fp);
	ShowStatus("'"CL_WHITE"attr_fix.txt"CL_RESET"' 에서 속성정보를 읽었습니다.\n");
	...
	if(fp == NULL){
		...
		fclose(fp);
		ShowStatus("'"CL_WHITE"%s"CL_RESET"' 읽음\n","statpoint.txt");
	}
