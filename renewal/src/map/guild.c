...
// 긎깑긤궻EXP긌긿긞긘깄궻긲깋긞긘깄궸듫쁀궥귡믦릶
#define GUILD_SEND_XY_INVERVAL	5000	// 띆뷭귘괾굊몭륪궻듩둼
#define GUILD_PAYEXP_INVERVAL 10000	// 듩둼(긌긿긞긘깄궻띍묈맯뫔렄듩갂?깏뷳)
#define GUILD_PAYEXP_LIST 8192	// 긌긿긞긘깄궻띍묈릶
...
int guild_read_guildskill_tree_db(void)
{
	...
	fclose(fp);
	ShowStatus("'"CL_WHITE"guild_skill_tree.txt"CL_RESET"' 에서 '"CL_WHITE"%d"CL_RESET"' 개의 자료를 읽었습니다.\n", ln);
...
static int guild_read_castledb(void)
{
	...
	fclose(fp);
	ShowStatus("'"CL_WHITE"castle_db.txt"CL_RESET"' 에서 '"CL_WHITE"%d"CL_RESET"' 개의 자료를 읽었습니다.\n", ln);
	return 0;
}
...
int guild_created(int account_id,int guild_id)
{
	...
	if(sd==NULL)
		return 0;
	if(!guild_id) {
		clif_guild_created(sd,2);	// 띿맟렪봲걁벏뼹긎깑긤뫔띪걂
		return 0;
	}
	//struct guild *g;
	...
	if(battle_config.guild_emperium_check)
		pc_delitem(sd,pc_search_inventory(sd,714),1,0);	// 긄깛긻깏긂?뤑뽙
	return 0;
}
...
int guild_recv_info(struct guild *sg)
{
	...
	for(i=0;i<g->max_member;i++){	// 륃뺪궻몭륪
		sd = g->member[i].sd;
		if( sd==NULL )
			continue;
		...
		if(	before.guild_lv!=g->guild_lv || bm!=m ||
			...
			clif_guild_basicinfo(sd);	// 딈?륃뺪몭륪
			clif_guild_emblem(sd,g);	// 긄깛긳깒?몭륪
		}
		...
		if( before.skill_point!=g->skill_point)
			clif_guild_skillinfo(sd);	// 긚긌깑륃뺪몭륪

		if( guild_new ){	// 뼟몭륪궶귞룋뫌륃뺪귖몭귡
...
int guild_invite(struct map_session_data *sd,struct map_session_data *tsd)
{
	...
	if(!battle_config.invite_request_check) {
		if (tsd->party_invite>0 || tsd->trade_partner || tsd->adopt_invite ) {	// 몜롨궕롦덙뭷궔궵궎궔
...
int guild_reqalliance(struct map_session_data *sd,struct map_session_data *tsd)
{
	...
	for(i=0;i<MAX_GUILDALLIANCE;i++){	// 궥궳궸벏뼼륉뫴궔둴봃
...
int guild_reply_reqalliance(struct map_session_data *sd,int account_id,int flag)
{
	...
	if(sd->guild_alliance!=tsd->status.guild_id)	// 뒰뾘궴긎깑긤ID궕댾궎
		return 0;

	if(flag==1){	// 뤂묡
		int i;

		struct guild *g,*tg;	// 벏뼼릶띋둴봃
		g=guild_search(sd->status.guild_id);
...
int guild_opposition(struct map_session_data *sd,struct map_session_data *tsd)
{
	...
	for(i=0;i<MAX_GUILDALLIANCE;i++){	// 궥궳궸듫똚귩렃궯궲궋귡궔둴봃
		if(g->alliance[i].guild_id==tsd->status.guild_id){
			if(g->alliance[i].opposition==1){	// 궥궳궸밎뫮
...
int guild_allianceack(int guild_id1,int guild_id2,int account_id1,int account_id2,int flag,const char *name1,const char *name2)
{
	...
	}else if((flag&0x0f)==1){	// 밎뫮믅뭢
		if( sd[0]!=NULL )
			clif_guild_oppositionack(sd[0],0);
	}


	for(i=0;i<2-(flag&1);i++){	// 벏뼼/밎뫮깏긚긣궻띋몭륪
		struct map_session_data *sd;
		if(g[i]!=NULL)
...
int guild_broken_sub(DBKey key,void *data,va_list ap)
{
	...
	for(i=0;i<MAX_GUILDALLIANCE;i++){	// 듫똚귩봨딙
...
int guild_broken(int guild_id,int flag)
{
	...
	for(i=0;i<g->max_member;i++){	// 긎깑긤됶랼귩믅뭢
		if((sd=g->member[i].sd)!=NULL){
...
{	// Run All NPC_Event[OnAgitStart]
	...
	ShowStatus("스크립트 이벤트:[OnAgitStart] 가 @공성시작 으로부터 (%d) 실행\n",c);
	// Start auto saving
...
{	// Run All NPC_Event[OnAgitEnd]
	...
	ShowStatus("스크립트 이벤트:[OnAgitEnd] 가 @공성종료 로부터 (%d) 실행\n",c);
	// Stop auto saving
...
{	// Run All NPC_Event[OnAgitStart2]
	...
	ShowStatus("스크립트 이벤트:[OnAgitStart2] 가 @공성시작2 으로부터 (%d) 실행\n",c);
	// Start auto saving
...
{	// Run All NPC_Event[OnAgitEnd2]
	...
	ShowStatus("스크립트 이벤트:[OnAgitEnd2] 가 @공성종료2 로부터 (%d) 실행\n",c);
	// Stop auto saving
