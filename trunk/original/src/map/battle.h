...
// 뫌맜?걁벶귒뜛귒궼pc.c갂battle_attr_fix궳럊뾭걂
...
//iLAthena additions
	int keepitem_use;	// 차암 시스템 사용여부 [★orz。]
	int immediateitem_use;	// 즉시 효과 아이템 사용여부 [★orz。]
	int probcard_use;	// 카드 확률 삽입 시스템 사용여부 [★orz。]
	int drop_item_identify;
	int party_exp_share;
	int save_warp_map;
	int charkami_useitem;
	int charkami_usemoney;
	int charkami_useitemcode;
	int charkami_grantlevel;
	int charkami_useitemamount;
} battle_config;
