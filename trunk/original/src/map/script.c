...
static struct {
	...
	int curly_count;	// 덭긇긞긓궻릶
	int index;			// 긚긏깏긵긣볙궳럊뾭궢궫?빒궻릶
} syntax;
...
const char* parse_callfunc(const char* p, int require_paren)
{
	...
	if( str_data[func].type == C_FUNC ){
		...
		disp_error_message("스크립트오류: 명령어사용 또는 문법오류",p);
...
const char* parse_subexpr(const char* p,int limit)
{
	...
	while((
		...
		if(op == C_OP3) {
			...
			p=skip_space(p);
			if( *(p++) != ':')
				disp_error_message("':' 기호가 필요함", p-1);
			p=parse_subexpr(p,-1);
		} else {
		...
		if(p2 - p == 4 && !strncasecmp(p,"case",4)) {
			...
			while(pos >= 0) {
				if(syntax.curly[pos].type == TYPE_DO) {
					...
					syntax.curly[pos].flag = 1; // continue 뾭궻깏깛긏뮗귡긲깋긐
					break;
				} else if(syntax.curly[pos].type == TYPE_FOR) {
...
const char* parse_syntax_close(const char *p) {
	// if(...) for(...) hoge(); 궻귝궎궸갂괦뱗빧궣귞귢궫귞띋뱗빧궣귞귢귡궔둴봃궥귡
	int flag;
...
void script_error(const char* src, const char* file, int start_line, const char* error_msg, const char* error_pos)
{
	...
	StringBuf_Printf(&buf, "%s 스크립트 %d 줄 오류\n", file, line);
...
BUILDIN_FUNC(warp)
{
	...
	// iLAthena 수정
		...
		if(battle_config.save_warp_map){
			pc_setglobalreg_str(sd,"PREV_MAP$", str);
			pc_setglobalreg(sd,"PREV_MAP_X", x);
			pc_setglobalreg(sd,"PREV_MAP_Y", y);
...
BUILDIN_FUNC(getscrate)
{
	...
	type=script_getnum(st,2);
	rate=script_getnum(st,3);
	if( script_hasdata(st,4) ) //럚믦궢궫긌긿깋궻뫯맜귩똶럁궥귡
...
BUILDIN_FUNC(globalmes)
{
	...
	mes=script_getstr(st,2);	// 긽긞긜?긙궻롦벦
	if(mes==NULL) return 0;
	
	if(script_hasdata(st,3)){	// NPC뼹궻롦벦(123#456)
		name=script_getstr(st,3);
	} else {
		name=nd->name;
	}

	npc_globalmessage(name,mes);	// 긐깓?긫깑긽긞긜?긙몭륪
...
BUILDIN_FUNC(successremovecards)
{
	...
	for( c = sd->inventory_data[i]->slot - 1; c >= 0; --c )
	{
		...
		{// extract this card from the item
			...
			if((flag=pc_additem(sd,&item_tmp,1))){	// 렃궲궶궋궶귞긤깓긞긵
				clif_additem(sd,0,0,flag);
	...
	{	// 긇?긤귩롦귟룣궋궫귺귽긡?룋벦
		...
		if((flag=pc_additem(sd,&item_tmp,1))){	// 귖궲궶궋궶귞긤깓긞긵
			clif_additem(sd,0,0,flag);
			map_addflooritem(&item_tmp,1,sd->bl.m,sd->bl.x,sd->bl.y,0,0,0,0);
		}
...
BUILDIN_FUNC(npcwhisper)
{
	...
	if(nd) {
		...
		// 귓말 보내기
		intif_wis_messageex(nd->name,pl_sd->status.name,(char *)msg,strlen(msg)+1);
		ShowDebug("npcwhisper: 귓말 보내기. NPC(%s) -> PC(%s) 에게\n",nd->name,pl_sd->status.name);
		return 0;
	}

	// ND 에러
	ShowError("npcwhisper: NPC Data 가져오기 실패.\n");
	return 0;
}
...
BUILDIN_FUNC(writemail)
{

    struct mail_message msg;
    struct party_data *p;
    struct guild *g;
    const char* target,*source,*title,*message;
    TBL_PC dummy_sd, *sd;
    int flag,i;
    msg.item.attribute=0;
    msg.item.identify=0;
    msg.id = 0;
    msg.timestamp=0;
//----------------------------------------
    source = script_getstr(st,2);
    target = script_getstr(st,3);
    title=script_getstr(st,4);
    message=script_getstr(st,5);
    flag=script_getnum(st,6);
    msg.zeny=script_getnum(st,7);
    msg.item.nameid=script_getnum(st,8);
    msg.item.amount=script_getnum(st,9);
    msg.item.refine=script_getnum(st,10);
    msg.item.card[0]=script_getnum(st,11);
    msg.item.card[1]=script_getnum(st,12);
    msg.item.card[2]=script_getnum(st,13);
    msg.item.card[3]=script_getnum(st,14);
//-----------------------------------------

    if(flag==3) {if((map_nick2sd((char *) target)) == NULL) {script_pushint(st,1); return 1;}}
    sd = &dummy_sd;
    safestrncpy(msg.send_name, source, NAME_LENGTH);
    safestrncpy(msg.dest_name, target, NAME_LENGTH);
    safestrncpy(msg.title, title, MAIL_TITLE_LENGTH);
    safestrncpy(msg.body, message, MAIL_BODY_LENGTH);
    
    switch(flag){
    case 1:     
                p=party_searchname(target);
                    if(p==NULL){script_pushint(st,1); return 0;}
                        for(i=0;i<MAX_PARTY;i++){    
                            if(p->party.member[i].account_id){
                                    safestrncpy(msg.dest_name, p->party.member[i].name, NAME_LENGTH);
                                    #ifndef TXT_ONLY
    ...
    case 2: 
                    g=guild_searchname((char*)target);
                    if(g==NULL){script_pushint(st,1); return 0;}
                        for(i=0;i<MAX_GUILD;i++){    
                            if(g->member[i].account_id){
                                    safestrncpy(msg.dest_name, g->member[i].name, NAME_LENGTH);
                                    #ifndef TXT_ONLY
                                    intif_Mail_send(sd->status.account_id, &msg);
                                    #endif
                                    }}break;
    ...
    case 4:
                    ...
                    for (i=0;i<users;i++){
                    safestrncpy(msg.dest_name, pl_allsd[i]->status.name, NAME_LENGTH);
                    #ifndef TXT_ONLY
                    intif_Mail_send(sd->status.account_id, &msg);
                    #endif
                    }break;
...
/*==========================================
 * 가짜 캐스팅바 로드 [★orz。]
 * usage: fakecasting( < CastingTime > {, < Skill ID > } );
 *------------------------------------------*/
BUILDIN_FUNC(fakecasting)
{
	TBL_PC * sd = map_id2sd(st->rid);	// 1.01 수정
	struct block_list *bl;	// 1.01 추가
	int time = script_getnum(st, 2);
	short skillid=0;
	// struct block_list *bl = &sd->bl;	// 1.02 제거
	...
	if( script_hasdata(st,3) )
		skillid = script_getnum(st,3);

	clif_skillcasting(bl, bl->id, 0, 0, 0, skillid, 0, time);

	// 1.03 추가 시작 - 소스발췌: sleep
	if( time <= 0 )
	{// do nothing
	}
	else if( st->sleep.tick == 0 )
...
BUILDIN_FUNC(plagiarism)
{
	...
	if (skillid <= 0 || skilllv <= 0) {
		ShowError("script(plagiarism) Skill id(%d), Skill lv(%d)\n", skillid, skilllv);
		script_pushint (st, -1);
		return -1;
	}

	if (pc_checkskill (sd,RG_PLAGIARISM) > 0) {
		if (sd->cloneskill_id && sd->status.skill[sd->cloneskill_id].flag == 13) {
		...
		sd->status.skill[skillid].lv = skilllv;
		if ((type = pc_checkskill(sd,RG_PLAGIARISM)) < skilllv)
			sd->status.skill[skillid].lv = type;
		sd->status.skill[skillid].flag = 13;//cloneskill flag
		pc_setglobalreg(sd, "CLONE_SKILL", sd->cloneskill_id);
		pc_setglobalreg(sd, "CLONE_SKILL_LV", sd->status.skill[skillid].lv);
		clif_skillinfoblock(sd);
	} else {
		script_pushint (st, 1);
		return 0;
...
/*==========================================
 * NPC 채팅방상태변경[★orz。]
 * changewaitingroom "<title>", <limit>, "<npc_name>";
 * changewaitingroom "<title>", <limit>;
 *------------------------------------------*/
BUILDIN_FUNC(changewaitingroom)
{
	struct npc_data* nd;
	struct chat_data* cd;
	const char *title;
	...
	if (nd != NULL && (cd=(struct chat_data *)map_id2bl(nd->chat_id)) != NULL)
		chat_changenpcchatstatus(nd, title, limit, pub);
	else if (nd == NULL)
		ShowDebug("script:changewaitingroom: npc %d not found\n", st->oid);
	else if (cd == NULL)
		ShowDebug("script:changewaitingroom: npc %d is not founded waiting room\n", st->oid);
	...
	if(g!=NULL){
		for(i=0;i<MAX_GUILD;i++){
			if(g->member[i].account_id){
				switch (type) {
				case 2:
					mapreg_setreg(add_str("$@guildmemberaid")+(j<<24),g->member[i].account_id);
					break;
				case 1:
					mapreg_setreg(add_str("$@guildmembercid")+(j<<24),g->member[i].char_id);
					break;
				default:
					mapreg_setregstr(add_str("$@guildmembername$")+(j<<24),g->member[i].name);
				}
				j++;
			}
		}
...
BUILDIN_FUNC(dmes)
{
	...
	if(script_getnum(st,3) == 1) {
		...
		clif_disp_onlyself(sd,str,strlen(str));
	}
	return 0;
}
...
// <--- [zBuffer] List of mob control commands
	...
	/*==========================================
	* iLAthena custom functions
	*------------------------------------------*/
	BUILDIN_DEF(mobspawn,"*"), // 몬스터 컨트롤
	BUILDIN_DEF(mobremove,"i"),
	BUILDIN_DEF(getmobdata,"i*"),
	BUILDIN_DEF(setmobdata,"iii"),
	BUILDIN_DEF(mobassist,"i?"),
	BUILDIN_DEF(mobattach,"i?"),
	BUILDIN_DEF(writemail,"ssssi*"), // 메일작성
	BUILDIN_DEF(areamobuseskill,"siiiiiviiiii"),
	BUILDIN_DEF(plagiarism, "ii"), // 도작 [제작자 불명]
	BUILDIN_DEF(fakecasting, "i?"), // 가짜캐스팅바 [★orz。]
	BUILDIN_DEF(changewaitingroom,"si?"), // NPC 채팅방상태변경[★orz。]
	BUILDIN_DEF(getguildmember, "i*"),
	BUILDIN_DEF(dmes, "s?"),
	BUILDIN_DEF(npcwhisper,"s*"), // [JaVb]
	/*-----------------------------------------*/
