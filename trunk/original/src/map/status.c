...
static unsigned short status_calc_speed(struct block_list *bl, struct status_change *sc, int speed)
{
	...
	else
	{
		...
		//GetMoveSlowValue()
		{
			...
			else
			{
				...
				if( sd && sd->speed_rate + sd->speed_add_rate > 0 ){ // permanent item-based speedup
					if(sd->speed_rate > 0)
						val = max( val, sd->speed_rate);
					if(sd->speed_add_rate > 0)
						val += sd->speed_add_rate;
				}
			}
		...
		//GetMoveHasteValue1()
		{
			...
			if( sc->data[SC_SPEEDUP0] ) // temporary item-based speedup
				val = max( val, 25 );
			// iLAthena ｼ､
			if( sd){ // permanent item-based speedup
				if(sd->speed_rate < 0)
					val = max( val, -(sd->speed_rate) );
				if(sd->speed_add_rate < 0)
					val += -(sd->speed_add_rate);
	...
	//GetSpeed()
	{
		...
		if( sc->data[SC_DEFENDER] )
			speed = max(speed, 200);
		if( sc->data[SC_WALKSPEED] ) // iLAthena ｼ､
...
int status_readdb(void)
{
	...
	fclose(fp);
	ShowStatus("'"CL_WHITE"%s"CL_RESET"' ﾀﾚｷ盧ｦ ﾀﾐｾ悅ﾀｴﾏｴﾙ.\n",path);
