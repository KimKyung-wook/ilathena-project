...
int clif_setip(const char* ip)
{
	...
	ShowInfo("甘 辑滚 IP 林家 : '"CL_WHITE"%s"CL_RESET"' -> '"CL_WHITE"%s"CL_RESET"'.\n", ip, ip2str(map_ip, ip_str));
	return 1;
}
...
void clif_setbindip(const char* ip)
{
	...
	bind_ip = host2ip(ip);
	if (bind_ip) {
		ShowInfo("甘 辑滚 Bind IP 林家 : '"CL_WHITE"%s"CL_RESET"' -> '"CL_WHITE"%s"CL_RESET"'.\n", ip, ip2str(bind_ip, ip_str));
	} else {
		ShowWarning("Failed to Resolve Map Server Address! (%s)\n", ip);
	}
}
...
int clif_guild_memberlist(struct map_session_data *sd)
{
	...
	for(i=0,c=0;i<g->max_member;i++){
		...
		memset(WFIFOP(fd,c*104+34),0,50);	// 儊儌丠
...
/*==========================================
 * 007d 僋儔僀傾儞僩懁?僢僾撉傒崬傒姰椆
 * map怤擖帪偵昁梫側僨??傪慡偰憲傝偮偗傞
 *------------------------------------------*/
...
void clif_parse_GuildRequestInfo(int fd, struct map_session_data *sd)
{
	...
	switch( RFIFOL(fd,2) )
	{
	case 0:	// 僊儖僪婎?忣曬丄摨柨揋懳忣曬
		clif_guild_basicinfo(sd);
		clif_guild_allianceinfo(sd);
		break;
	case 1:	// 儊儞僶?儕僗僩丄栶怑柤儕僗僩
		clif_guild_positionnamelist(sd);
		clif_guild_memberlist(sd);
		break;
	case 2:	// 栶怑柤儕僗僩丄栶怑忣曬儕僗僩
		clif_guild_positionnamelist(sd);
...
int clif_parse(int fd)
{
	...
	if (session[fd]->flag.eof) {
		if (sd) {
			if (sd->state.autotrade) {
				...
				sd->fd = 0;
				ShowInfo("%s某腐磐 '"CL_WHITE"%s"CL_RESET"' 肺弊酒眶 (磊悼芭贰 荤侩).\n", (pc_isGM(sd))?"GM ":"", sd->status.name);
			} else
			if (sd->state.active) {
				 // Player logout display [Valaris]
				ShowInfo("%s某腐磐 '"CL_WHITE"%s"CL_RESET"' 肺弊酒眶.\n", (pc_isGM(sd))?"GM ":"", sd->status.name);
				clif_quitsave(fd, sd);
			} else {
				//Unusual logout (during log on/off/map-changer procedure)
				ShowInfo("敲饭捞绢 拌沥ID:%d/某腐磐ID:%d 肺弊酒眶.\n", sd->status.account_id, sd->status.char_id);
				map_quit(sd);
			}
		} else {
			ShowInfo("'"CL_WHITE"%s"CL_RESET"' 肺何磐 楷搬 辆丰.\n", ip2str(session[fd]->client_addr, NULL));
		}
		do_close(fd);
		return 0;
	}
	...
	if (sd) {
		...
		{// failed to identify packet version
			ShowInfo("楷搬 辆丰: 技记 #%d 舅荐绝绰 菩哦 滚傈 %s.\n", fd, (
				err == 1 ? "" :
...
static int packetdb_readdb(void)
{
...
#elif PACKETVER < 20071106	// 78-7b 婽搰埲? lv99僄僼僃僋僩梡
	...
	ShowStatus("'"CL_WHITE"packet_db.txt"CL_RESET"' 颇老肺何磐 '"CL_WHITE"%d"CL_RESET"' 滚傈狼 菩哦阑 佬菌嚼聪促.\n", clif_config.packet_db_ver);
	return 0;
}
