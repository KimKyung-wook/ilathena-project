...
static void mob_item_drop(struct mob_data *md, struct item_drop_list *dlist, struct item_drop *ditem, int loot, int drop_rate, unsigned short flag)
{
	...
	if( sd
		...
		&& ((itemdb_search(ditem->item_data.nameid)->imd.forcedrop) == 0)	// 즉시효과아이템 [★orz。]
...
void mob_damage(struct mob_data *md, struct block_list *src, int damage)
{
	...
	if (battle_config.show_mob_info&8){
		...
		switch(src->type){
			case BL_PC:  sd = (struct map_session_data *)src; break;
		...
		if(sd && sd != NULL){
			if(sd->status.party_id){
				...
				struct map_session_data* tsd[MAX_PARTY];
				int i, c;
				for (i = c = 0; i < MAX_PARTY; i++) {
					if( (tsd[c] = p->data[i].sd) == NULL || tsd[c]->bl.m != src->m)
						continue;
					c++;
				}
				for (i = 0; i < c; i++)
					clif_hpmeter_single(tsd[i]->fd, md->bl.id, md->status.hp, md->status.max_hp);
			}
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	{ // Item Drop
		...
		if(mvp_sd){
			int i, n, max;
			struct item_global_drop *gd;

			max = itemdb_globaldrop_max();

			for(i=0; i< max; i++){
				gd = itemdb_getglobaldrop(i);
				if(!gd)
					continue;
				if(rand()%10000 >= gd->p || gd->p <= 0)
					if (!(mvp_sd && mvp_sd->sc.data[SC_ITEMBOOST] && rand() % 10000 < drop_rate))
						continue;
				for(n=0; n<7; n++){
					switch(gd->condition[n].cond){
						case GDC_MOBID:		if(mobdb_searchname(md->name) != gd->condition[n].val)	break;
						case GDC_RACE:		if(md->status.race != gd->condition[n].val)				break;
						case GDC_SIZE:		if(md->status.size != gd->condition[n].val)				break;
						case GDC_ELE:		if(md->status.def_ele != gd->condition[n].val)			break;
						case GDC_MINLV:		if(mvp_sd->status.base_level < gd->condition[n].val)	break;
						case GDC_MAXLV:		if(mvp_sd->status.base_level > gd->condition[n].val)	break;
						case GDC_ALWAYS:
							mob_item_drop(md, dlist, mob_setdropitem(gd->nameid, 1), 0, gd->p, 0);
						break;
						default:
						break;
					}
	...
	{ // Only trigger event on final kill
		...
		{
			//pc_setglobalreg(mvp_sd,"killedrid",md->class_); // 삭제 [사이버짱]
...
/*==========================================
 *MOBskill궔귞둤뱰skillid궻skillidx귩뺅궥
 *------------------------------------------*/
int mob_skillid2skillidx(int class_,int skillid)
{
...
static int mob_readdb_mobavail(void)
{
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if(mob_db(class_) == mob_dummy)	// 뭠궕댶륂궶귞룉뿚궢궶궋갃
			continue;
	...
	fclose(fp);
	ShowStatus("'"CL_WHITE"mob_avail.txt"CL_RESET"' 에서 '"CL_WHITE"%d"CL_RESET"' 개의 자료를 읽었습니다.\n", ln);
	return 0;
}
...
static int mob_read_randommonster(void)
{
	...
	{
		mob_db_data[0]->summonper[i] = 1002;	// 먠믦궢뻒귢궫뤾뜃궼?깏깛궕뢯귡귝궎궸궢궲궓궘
		...
		fclose(fp);
		ShowStatus("'"CL_WHITE"%s"CL_RESET"' 자료를 읽었습니다.\n", mobfile[i]);
	}
	return 0;
}
...
static int mob_readdb_race(void)
{
	...
	fclose(fp);
	ShowStatus("'"CL_WHITE"mob_race2_db.txt"CL_RESET"' 자료를 읽었습니다.\n");
	return 0;
}
