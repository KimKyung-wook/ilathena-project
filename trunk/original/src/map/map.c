...
int event_tid=0; // @¿Ã∫•∆Æ ∏Ì∑…æÓ [JaVb]
int event_tick=0; // @¿Ã∫•∆Æ ∏Ì∑…æÓ [JaVb]
int event_rate=0; // @¿Ã∫•∆Æ ∏Ì∑…æÓ [JaVb]
int event_dflag=0; // @¿Ã∫•∆Æ ∏Ì∑…æÓ [JaVb]
int event_eflag=0; // @¿Ã∫•∆Æ ∏Ì∑…æÓ [JaVb]
 ...
 * ÉçÉbÉNÇ≥ÇÍÇƒÇ¢ÇÈÇ∆Ç´ÇÕÉoÉbÉt?Ç…ÇΩÇﬂÇÈ
 *------------------------------------------*/
 ...
 * ÉoÉbÉt?Ç…ÇΩÇ‹Ç¡ÇƒÇ¢ÇΩblockÇëSïîçÌèú
 *------------------------------------------*/
...
int map_foreachinrange(int (*func)(struct block_list*,va_list), struct block_list* center, int range, int type, ...)
{
	...
	map_freeblock_lock();	// ÉÅÉÇÉäÇ©ÇÁÇÃâï˙Çã÷?Ç∑ÇÈ

	for(i=blockcount;i<bl_list_count;i++)
		if(bl_list[i]->prev)	// óL?Ç©Ç«Ç§Ç©?ÉFÉbÉN
		{
			va_list ap;
			va_start(ap, type);
...
/*==========================================
 * ãÈ?(x0,y0)-(x1,y1)Ç™(dx,dy)à⁄ìÆÇµÇΩéb?
 * óÃàÊäOÇ…Ç»ÇÈóÃàÊ(ãÈ?Ç©Léö?)?ÇÃobjÇ…
 * ?ÇµÇƒfuncÇåƒÇ‘
 *
 ...
 * è∞ÉAÉCÉe?Çè¡Ç∑
 *
 * data==0ÇÃéb?imerÇ≈è¡Ç¶ÇΩéÍ * data!=0ÇÃéb?EÇ§ìôÇ≈è¡Ç¶ÇΩéb?µÇƒìÆç?
 *
...
/*==========================================
 * (m,x,y)ÇíÜêSÇ…3x3à»?Ç…è∞ÉAÉCÉe?ê›íu
 *
...
void map_spawnmobs(int m)
{
	...
	{
		ShowStatus("%s ¡ˆø™ø° '"CL_WHITE"%d"CL_RESET"' ∏ÛΩ∫≈Õ ª˝º∫\n",map[m].name, k);
	}
}
...
int map_readallmaps (void)
{
	...
	else
	{
		...
		{
			ShowFatalError(CL_WHITE"%s"CL_RESET" æ¯¿Ω\n", map_cache_file);
	...
	for(i = 0; i < map_num; i++)
	{
		...
		// show progress
		ShowStatus("[%i/%i]: %s ∏ ¿ª ¿–∞Ì ¿÷Ω¿¥œ¥Ÿ."CL_CLL"\r", i, map_num, map[i].name);
	...
	ShowInfo(CL_WHITE"%d"CL_RESET" ∞≥¿« ∏ ¿ª ¿–æ˙Ω¿¥œ¥Ÿ."CL_CLL"\n",map_num);
...
int inter_config_read(char *cfgName)
{
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		if(strcmpi(w1,"use_sql_db")==0) {
			...
			ShowStatus ("SQL µ•¿Ã≈Õ∫£¿ÃΩ∫ ªÁøÎ: %s\n",w2);
		} else
...
{
	ShowStatus("∏  µ•¿Ã≈Õ∫£¿ÃΩ∫ ø¨∞·¡æ∑·\n");
	Sql_Free(mmysql_handle);
	mmysql_handle = NULL;

	if (log_config.sql_logs)
	{
		ShowStatus("±‚∑œ µ•¿Ã≈Õ∫£¿ÃΩ∫ ø¨∞·¡æ∑·\n");
...
int log_sql_init(void)
{
	...
	ShowInfo(""CL_WHITE"[SQL]"CL_RESET": ±‚∑œ µ•¿Ã≈Õ∫£¿ÃΩ∫ "CL_WHITE"%s"CL_RESET" ø¨∞·¡ﬂ "CL_WHITE"%s"CL_RESET"...\n",log_db_db,log_db_ip);
	if ( SQL_ERROR == Sql_Connect(logmysql_handle, log_db_id, log_db_pw, log_db_ip, log_db_port, log_db_db) )
		exit(EXIT_FAILURE);
	ShowStatus(""CL_WHITE"[SQL]"CL_RESET": '"CL_WHITE"%s"CL_RESET"' µ•¿Ã≈Õ∫£¿ÃΩ∫ '"CL_GREEN"ø¨∞·º∫∞¯"CL_RESET"'.\n", log_db_db);
...
void do_abort(void)
{
	...
	if (run) {
		ShowFatalError("ƒ≥∏Ø≈Õ¡§∫∏ ¿˙¿Â ¡ﬂ ø¿∑˘πﬂª˝. ƒ≥∏Ø≈Õ ¿˙¿Â∫“∞°!\n");
		return;
	}
	run = 1;
	...
	{
		if (pc_db->size(pc_db))
			ShowFatalError("ƒ≥∏Ø≈Õ º≠πˆø° ø¨∞·¿Ã µ«¡ˆæ æ∆ ø¿∑˘πﬂª˝. %u ƒ≥∏Ø≈Õ ¿˙¿Â∫“∞°!\n", pc_db->size(pc_db));
		return;
	}
	ShowError("º≠πˆø¿∑˘Ω≈»£ ºˆΩ≈µ . ¡¢º”ªÛ≈¬¿« ∏µÁƒ≥∏Ø≈Õ¡§∫∏¿˙¿Â¿ª Ω√µµ«‘!\n");
...
{
	ShowInfo("CL_WHITE" "iLAthena πˆ¿¸ %d.%02d.%02d, Athena Mod πˆ¿¸ %d" CL_RESET"\n",
		ATHENA_MAJOR_VERSION, ATHENA_MINOR_VERSION, ATHENA_REVISION,
		ATHENA_MOD_VERSION);
	ShowInfo(CL_GREEN "ªÁ¿Ã∆Æ/∆˜∑≥:" CL_RESET "\thttp://www.i-luna.com/");
	ShowInfo(CL_GREEN "IRC √§≥Œ:" CL_RESET "\tirc://irc.hanirc.org/#æ∆¿Ã∑Á≥™");
	if (flag) exit(EXIT_FAILURE);
}
...
int do_init(int argc, char *argv[])
{
	...
	if (!map_ip_set || !char_ip_set) {
		...
		ShowWarning("IP¡÷º“ ¿⁄µøº≥¡§ (IP∞¯¿Ø±‚/VPN ≥ª∫Œ»Ø∞ÊΩ√ IP¡÷º“ ºˆµøº≥¡§ « ø‰)\n");

		if (naddr_ == 0)
			ShowError("Unable to determine your IP address...\n");
		else if (naddr_ > 1)
			ShowNotice("ø©∑Ø∞≥¿« ≥◊∆Æøˆ≈© ∞Àªˆ ±◊¡ﬂ %s IP¡÷º“ ªÁøÎ\n");
	...
	if (battle_config.pk_mode == 1)
		ShowNotice("º≠πˆ∞° '"CL_WHITE"PK ∏µÂ"CL_RESET"' ∑Œ Ω««‡\n");

	ShowStatus(CL_BG_GREEN""CL_WHITE"∏  º≠πˆ Ω««‡ øœ∑·\n"CL_NORMAL);
