	...
	if (!message || !*message ||
		(sscanf(message, "%15s %d %d", map_name, &x, &y) < 3 &&
		 sscanf(message, "%15[^,],%d,%d", map_name, &x, &y) < 1)) {
			...
			clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@이동/@맵/@워프 <맵이름> <X좌표> <Y좌표>)");
			return -1;
	}
...
int atcommand_where(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@어디 <캐릭터이름>)");
		return -1;
	}
...
int atcommand_jumpto(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@출두/@점프투/@플레이어 <캐릭터이름>)");
		return -1;
	}
	...
	if (pl_sd == sd)
	{
		clif_displaymessage(fd, "자신에게로는 이동할 수 없습니다.");
		return -1;
	}
		...
		{// you can look only lower or same level
			...
			if (strstr(player_name, match_text) != NULL) { // search with no case sensitive
				...
				if (battle_config.who_display_aid > 0 && pc_isGM(sd) >= battle_config.who_display_aid) {
					sprintf(atcmd_output, "(캐릭터ID:%d/계정ID:%d) ", pl_sd->status.char_id, pl_sd->status.account_id);
				} else {
					atcmd_output[0]=0;
				}
				//Player name
...
int atcommand_whomap3(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
	{
		...
		if (pl_GM_level > 0)
			sprintf(atcmd_output, "이름: %s (GM:%d) | 위치: %s %d %d", pl_sd->status.name, pl_GM_level, mapindex_id2name(pl_sd->mapindex), pl_sd->bl.x, pl_sd->bl.y);
		else
			sprintf(atcmd_output, "이름: %s | 위치: %s %d %d", pl_sd->status.name, mapindex_id2name(pl_sd->mapindex), pl_sd->bl.x, pl_sd->bl.y);
		clif_displaymessage(fd, atcmd_output);
		count++;
	}
	mapit_free(iter);
...
int atcommand_whomap2(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
	{
		...
		if (pl_GM_level > 0)
			sprintf(atcmd_output, "이름: %s (GM:%d) | 레벨: %d | 직업: %s (레벨: %d)", pl_sd->status.name, pl_GM_level, pl_sd->status.base_level, job_name(pl_sd->status.class_), pl_sd->status.job_level);
		else
			sprintf(atcmd_output, "이름: %s | 레벨: %d | 직업: %s (레벨: %d)", pl_sd->status.name, pl_sd->status.base_level, job_name(pl_sd->status.class_), pl_sd->status.job_level);
		clif_displaymessage(fd, atcmd_output);
		count++;
	}
	mapit_free(iter);
...
int atcommand_whomap(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
	{
		...
		if (pl_GM_level > 0)
			sprintf(atcmd_output, "이름: %s (GM:%d) | 파티: '%s' | 길드: '%s'", pl_sd->status.name, pl_GM_level, temp0, temp1);
		else
			sprintf(atcmd_output, "이름: %s | 파티: '%s' | 길드: '%s'", pl_sd->status.name, temp0, temp1);
		clif_displaymessage(fd, atcmd_output);
		count++;
	}
	mapit_free(iter);
	...
	if (count == 0)
		...
		sprintf(atcmd_output, msg_txt(56), map[map_id].name, count); // iLAthena 순서 수정
	}
	clif_displaymessage(fd, atcmd_output);
	...
	for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
	{
		...
		if (pl_GM_level > GM_level) {
			if (pl_sd->sc.option & OPTION_INVISIBLE)
				continue;
			sprintf(atcmd_output, "이름: %s (GM)", pl_sd->status.name);
			clif_displaymessage(fd, atcmd_output);
			count++;
			continue;
		}

		sprintf(atcmd_output, "이름: %s (GM:%d) | 위치: %s %d %d",
			pl_sd->status.name, pl_GM_level,
			mapindex_id2name(pl_sd->mapindex), pl_sd->bl.x, pl_sd->bl.y);
		clif_displaymessage(fd, atcmd_output);

		sprintf(atcmd_output, "       레벨: %d | 직업: %s (레벨: %d)",
		...
		sprintf(atcmd_output,"       파티: '%s' | 길드: '%s'",
			p?p->party.name:"없음", g?g->name:"없음");
...
int atcommand_speed(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &speed) < 1) {
		sprintf(atcmd_output, "명령어 사용 방법이 잘못 되었습니다. (@속도 <값>)", MIN_WALK_SPEED, MAX_WALK_SPEED);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_storage(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	clif_displaymessage(fd, "창고를 열었습니다.");
...
int atcommand_guildstorage(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	clif_displaymessage(fd, "길드창고를 열었습니다.");
	return 0;
}
...
int atcommand_option(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d %d", &param1, &param2, &param3) < 1 || param1 < 0 || param2 < 0 || param3 < 0) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@옵션 <값1> <값2> <값3>)");
		return -1;
	}
...
int atcommand_jobchange(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d", &job, &upper) < 1)
	{
		...
		if (!found) {
			clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@직업/@전직 <직업코드>)");
			clif_displaymessage(fd, "   0 노비스         7 기사                14 크루세이더          21 크루세이더(페코)");
			clif_displaymessage(fd, "   1 검사           8 프리스트            15 몽크");
			clif_displaymessage(fd, "   2 마법사         9 위저드              16 세이지              23 슈퍼노비스");
			clif_displaymessage(fd, "   3 궁수          10 블랙스미스          17 로그                24 건슬링거");
			clif_displaymessage(fd, "   4 복사          11 헌터                18 알케미스트          25 닌자");
			clif_displaymessage(fd, "   5 상인          12 어쌔신              19 바드");
			clif_displaymessage(fd, "   6 도둑          13 기사(페코)          20 댄서");
			clif_displaymessage(fd, "4001 상위노비스  4008 로드나이트        4015 팔라딘            4022 팔라딘(페코)");
			clif_displaymessage(fd, "4002 상위검사    4009 프리스트          4016 챔피온");
			clif_displaymessage(fd, "4003 상위마법사  4010 상위위저드        4017 프로페서");
			clif_displaymessage(fd, "4004 상위궁수    4011 화이트스미스      4018 스토커");
			clif_displaymessage(fd, "4005 상위복사    4012 스나이퍼          4019 크리에이터");
			clif_displaymessage(fd, "4006 상위상인    4013 어쌔신크로스      4020 클로운");
			clif_displaymessage(fd, "4007 상위도둑    4014 로드나이트(페코)  4021 집시");
			clif_displaymessage(fd, "4023 아기노비스  4030 아기기사          4037 아기크루세이더    4044 아기크루세이더(페코)");
			clif_displaymessage(fd, "4024 아기검사    4031 아기프리스트      4038 아기몽크          4045 아기슈퍼노비스");
			clif_displaymessage(fd, "4025 아기마법사  4032 아기위저드        4039 아기세이지        4046 태권");
			clif_displaymessage(fd, "4026 아기궁수    4033 아기블랙스미스    4040 아기로그          4047 권성");
			clif_displaymessage(fd, "4027 아기복사    4034 아기헌터          4041 아기알케미스트");
			clif_displaymessage(fd, "4028 아기상인    4035 아기어쌔신        4042 아기바드          4049 소울링커");
			clif_displaymessage(fd, "4029 아기도둑    4036 아기기사(페코)    4043 아기댄서");
			return -1;
		}
	}
...
int atcommand_kill(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@킬 <캐릭터이름/ID>");
		return -1;
	}
...
int atcommand_alive(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	{
		clif_displaymessage(fd, "사망한 상태가 아닙니다.");
		return -1;
	}
...
int atcommand_kami(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if(*(command + 5) != 'c' && *(command + 5) != 'C') {
		...
		if (!message || !*message) {
			clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@공지 <글>)");
			return -1;
		}
		...
		if(!message || !*message || (sscanf(message, "%lx %199[^\n]", &color, atcmd_output) < 2)) {
			clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@색공지 <색코드> <글>)");
			return -1;
		}
	
		if(color > 0xFFFFFF) {
			clif_displaymessage(fd, "색코드가 잘못 되었습니다.");
			return -1;
		}
...
int atcommand_item(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (
		...
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@아이템 <아이템이름/ID> <갯수>)");
		return -1;
	}
...
int atcommand_item2(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (
		...
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@아이템 <아이템이름/ID> <갯수>");
		clif_displaymessage(fd, "  <감정여부> <제련> <속성> <카드1> <카드2> <카드3> <카드4>)");
		return -1;
	}
...
int atcommand_baselevelup(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || !level) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@렙업/@렙/@레벨/@베렙/@베이스/@베이스레벨/@베이스렙 <레벨>)");
		return -1;
	}
...
int atcommand_joblevelup(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || !level) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@잡업/@잡렙/@잡레벨/@잡렙업/@직업레벨/@잡옵 <레벨>)");
		return -1;
	}
	if (level > 0) {
...
int atcommand_model(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d %d", &hair_style, &hair_color, &cloth_color) < 1) {
		sprintf(atcmd_output, "명령어 사용 방법이 잘못 되었습니다. (@모델 <머리스타일: %d-%d> <머리색: %d-%d> <옷색: %d-%d>)",
...
int atcommand_dye(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &cloth_color) < 1) {
		sprintf(atcmd_output, "명령어 사용 방법이 잘못 되었습니다. (@옷색/@옷염색 <%d-%d>).", MIN_CLOTH_COLOR, MAX_CLOTH_COLOR);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_hair_style(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &hair_style) < 1) {
		sprintf(atcmd_output, "명령어 사용 방법이 잘못 되었습니다. (@머리모양/@헤어스타일 <%d-%d>).", MIN_HAIR_STYLE, MAX_HAIR_STYLE);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_hair_color(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &hair_color) < 1) {
		sprintf(atcmd_output, "명령어 사용 방법이 잘못 되었습니다. (@머리색/@머리염색 <%d-%d>).", MIN_HAIR_COLOR, MAX_HAIR_COLOR);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_go(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if( map[sd->bl.m].flag.nogo && battle_config.any_warp_GM_min_level > pc_isGM(sd) ) {
		clif_displaymessage(sd->fd,"@마을 명령어를 사용할 수 없습니다.");
		return 0;
	}
	...
	if (!message || !*message || sscanf(message, "%11s", map_name) < 1 || town < 0 || town >= ARRAYLENGTH(data)) {
		...
		clif_displaymessage(fd, " 0=프론테라       1=모로크         2=게펜");
		clif_displaymessage(fd, " 3=폐이욘         4=알베르타       5=이즈루드");
		clif_displaymessage(fd, " 6=알데바란       7=루티에         8=코모도");
		clif_displaymessage(fd, " 9=유노          10=아마쯔        11=곤륜");
		clif_displaymessage(fd, " 12=움발라       13=니플헤임      14=용지성");
		clif_displaymessage(fd, " 15=초보자수련장 16=감옥          17=자와이");
		clif_displaymessage(fd, " 18=아요타야     19=아인브로크    20=리히타르젠");
		clif_displaymessage(fd, " 21=아인베흐     22=휘겔          23=라헬");
		clif_displaymessage(fd, " 24=베인스       25=모스코비아");
		return -1;
	}
...
int atcommand_monster(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (battle_config.etc_log)
		ShowInfo("%s 몬스터='%s' 이름='%s' 아이디=%d 개체수=%d (%d,%d)\n", command, monster, name, mob_id, number, sd->bl.x, sd->bl.y);
...
int atcommand_monstersmall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "몬스터이름 또는 ID 를 입력하여 주세요.");
		return -1;
	}
	...
	if (mobdb_checkid(mob_id) == 0) {
		clif_displaymessage(fd, "몬스터이름 또는 ID 가 잘못 입력 되었습니다."); // Invalid Monster ID.
		return -1;
	}
...
int atcommand_refine(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d", &position, &refine) < 2) {
		clif_displaymessage(fd, "장비위치와 제련도를 입력하여 주세요. (@제련 <장비위치> <+/- 제련도>)");
...
int atcommand_produce(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (
		...
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@제조 <장비이름/ID> <속성> <별조각>)");
		return -1;
	}
...
int atcommand_memo(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	{
		int i;
		clif_displaymessage(sd->fd,  "기억위치:");
	...
	{
		sprintf(atcmd_output, "명령어 사용 방법이 잘못 되었습니다. (@메모 <기억위치:%d-%d>).", 0, MAX_MEMOPOINTS-1);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_displaystatus(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (i = sscanf(message, "%d %d %d", &type, &flag, &tick)) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@displaystatus <타입> <플래그> <시간>)");
		return -1;
	}
	if (i < 2) flag = 1;
	if (i < 3) tick = 0;
...
int atcommand_statuspoint(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (point = atoi(message)) == 0) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@스텟포인트 <값>)");
		return -1;
	}
...
int atcommand_skillpoint(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (point = atoi(message)) == 0) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@스킬포인트 <값>)");
		return -1;
	}
...
int atcommand_zeny(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (zeny = atoi(message)) == 0) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@제니 <값>)");
		return -1;
	}
...
int atcommand_param(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &value) < 1 || value == 0) {
		sprintf(atcmd_output, "명령어 사용 방법이 잘못 되었습니다. (@힘/@어/@바/@인/@덱/@럭 <값>)");
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_guildlevelup(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &level) < 1 || level == 0) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@길업/길드레벨 <값>)");
		return -1;
	}
...
int atcommand_makeegg(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@펫생성 <ID>)");
		return -1;
	}
...
int atcommand_petfriendly(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (friendly = atoi(message)) < 0) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@펫친밀도 <0-1000>)");
		return -1;
	}
...
int atcommand_pethungry(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (hungry = atoi(message)) < 0) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@펫만복도 <0-100>)");
		return -1;
	}
...
int atcommand_recall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@소환 <캐릭터이름/ID>)");
		return -1;
	}
	...
	if (pl_sd == sd)
	{
		clif_displaymessage(fd, "자신을 소환할 수 없습니다.");
		return -1;
	}
...
int atcommand_char_block(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@캐릭블럭/@캐릭터블럭 <캐릭터이름>)");
		return -1;
	}
...
int atcommand_char_ban(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%s %23[^\n]", atcmd_output, atcmd_player_name) < 2) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@캐릭밴/@캐릭차단 <시간> <캐릭터이름>)");
		return -1;
	}
...
int atcommand_char_unblock(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@캐릭블럭해제/@캐릭터블럭해제 <캐릭터이름>)");
		return -1;
	}
...
int atcommand_char_unban(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@캐릭밴해제 <캐릭터이름>)");
		return -1;
	}
...
int atcommand_kick(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@킥 <캐릭터이름>)");
		return -1;
	}
...
int atcommand_questskill(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (skill_id = atoi(message)) < 0) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@퀘스킬 <스킬번호>)");
		return -1;
	}
...
int atcommand_lostskill(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || (skill_id = atoi(message)) < 0) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@퀘스킬2 <스킬번호>)");
		return -1;
	}
...
int atcommand_spiritball(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	{
		char msg[256];
		safesnprintf(msg, sizeof(msg), "명령어 사용 방법이 잘못 되었습니다.(@축기 <0-%d>)", max_spiritballs);
...
int atcommand_party(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", party) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@파티 <이름>)");
		return -1;
	}
...
int atcommand_guild(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", guild) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@길드 <길드이름>)");
		return -1;
	}
...
int atcommand_idsearch(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%99s", item_name) < 0) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@아이템검색 <아이템이름/ID>)");
		return -1;
	}
...
int atcommand_guildrecall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", guild_name) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@길드소환 <길드이름/ID>)");
		return -1;
	}
...
int atcommand_partyrecall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", party_name) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@파티소환 <파티이름/ID>)");
		return -1;
	}
...
int atcommand_mapinfo(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (list < 0 || list > 3) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@맵정보 <0-3> <맵이름>)");
		return -1;
	}
	...
	sprintf(atcmd_output, "이름: %s | 플레이어: %d | NPC: %d | 채팅: %d", mapname, map[m_id].users, map[m_id].npc_num, chat_num);
	clif_displaymessage(fd, atcmd_output);
	...
	if (map[m_id].flag.pvp)
		strcat(atcmd_output, "ON | ");
	if (map[m_id].flag.pvp_noguild)
		strcat(atcmd_output, "길드없음 | ");
	if (map[m_id].flag.pvp_noparty)
		strcat(atcmd_output, "파티없음 | ");
	if (map[m_id].flag.pvp_nightmaredrop)
		strcat(atcmd_output, "나이트메어 | ");
	if (map[m_id].flag.pvp_nocalcrank)
		strcat(atcmd_output, "랭크미계산 | ");
	clif_displaymessage(fd, atcmd_output);

	strcpy(atcmd_output,"GvG: ");
	if (map[m_id].flag.gvg)
		strcat(atcmd_output, "켜짐 | ");
	if (map[m_id].flag.gvg_dungeon)
		strcat(atcmd_output, "GvG 던전 | ");
	if (map[m_id].flag.gvg_castle)
		strcat(atcmd_output, "GvG 성 | ");
	if (map[m_id].flag.gvg_noparty)
		strcat(atcmd_output, "파티없음 | ");
	clif_displaymessage(fd, atcmd_output);

	strcpy(atcmd_output,"텔레포트: ");
	if (map[m_id].flag.noteleport)
		strcat(atcmd_output, "텔레포트 금지 | ");
	if (map[m_id].flag.monster_noteleport)
		strcat(atcmd_output, "몬스터 텔레포트 금지 | ");
	if (map[m_id].flag.nowarp)
		strcat(atcmd_output, "워프금지 | ");
	if (map[m_id].flag.nowarpto)
		strcat(atcmd_output, "이동금지 | ");
	if (map[m_id].flag.noreturn)
		strcat(atcmd_output, "복귀금지 | ");
	if (map[m_id].flag.nogo)
		strcat(atcmd_output, "NoGo | ");
	if (map[m_id].flag.nomemo)
		strcat(atcmd_output, "기억금지 | ");
	clif_displaymessage(fd, atcmd_output);

	sprintf(atcmd_output, "경험치 패널티 없음: %s | 제니 패널티 없음: %s", (map[m_id].flag.noexppenalty) ? "켜짐" : "꺼짐", (map[m_id].flag.nozenypenalty) ? "켜짐" : "꺼짐");
	clif_displaymessage(fd, atcmd_output);
	...
	strcpy(atcmd_output,"날씨: ");
	if (map[m_id].flag.snow)
		strcat(atcmd_output, "눈 | ");
	if (map[m_id].flag.fog)
		strcat(atcmd_output, "안개 | ");
	if (map[m_id].flag.sakura)
		strcat(atcmd_output, "벚꽃 | ");
	if (map[m_id].flag.clouds)
		strcat(atcmd_output, "구름 | ");
	if (map[m_id].flag.clouds2)
		strcat(atcmd_output, "구름2 | ");
	if (map[m_id].flag.fireworks)
		strcat(atcmd_output, "단풍 | ");
	if (map[m_id].flag.leaves)
		strcat(atcmd_output, "낙엽 | ");
	if (map[m_id].flag.rain)
		strcat(atcmd_output, "비 | ");
	if (map[m_id].flag.indoors)
		strcat(atcmd_output, "건물내부 | ");
	if (map[m_id].flag.nightenabled)
		strcat(atcmd_output, "밤 | ");
	clif_displaymessage(fd, atcmd_output);

	strcpy(atcmd_output,"기타: ");
	if (map[m_id].flag.nobranch)
		strcat(atcmd_output, "고목금지 | ");
	if (map[m_id].flag.notrade)
		strcat(atcmd_output, "거래금지 | ");
	if (map[m_id].flag.novending)
		strcat(atcmd_output, "노점금지 | ");
	if (map[m_id].flag.nodrop)
		strcat(atcmd_output, "드롭금지 | ");
	if (map[m_id].flag.noskill)
		strcat(atcmd_output, "스킬금지 | ");
	if (map[m_id].flag.noicewall)
		strcat(atcmd_output, "아이스월금지 | ");
		
	clif_displaymessage(fd, atcmd_output);

	strcpy(atcmd_output,"기타: ");
	if (map[m_id].nocommand)
		strcat(atcmd_output, "명령어금지 | ");
	if (map[m_id].flag.nobaseexp)
		strcat(atcmd_output, "경험치금지 | ");
	if (map[m_id].flag.nojobexp)
		strcat(atcmd_output, "잡경험치금지 | ");
	if (map[m_id].flag.nomobloot)
		strcat(atcmd_output, "몬스터 아이템 수집 금지 | ");
	if (map[m_id].flag.nomvploot)
		strcat(atcmd_output, "MVP아이템 수집 금지 | ");
	if (map[m_id].flag.partylock)
		strcat(atcmd_output, "파티잠금 | ");
	if (map[m_id].flag.guildlock)
		strcat(atcmd_output, "길드잠금 | ");
	clif_displaymessage(fd, atcmd_output);
	...
	case 0:
		...
		break;
	case 1:
		clif_displaymessage(fd, "----- 플레이어 -----");
		iter = mapit_getallusers();
		for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
		{
			if (pl_sd->mapindex == m_index) {
				sprintf(atcmd_output, "플레이어: %s (세션 #%d) | 위치: %d,%d",
		...
		break;
	case 2:
		clif_displaymessage(fd, "----- NPC -----");
		for (i = 0; i < map[m_id].npc_num;)
		{
			nd = map[m_id].npc[i];
			switch(nd->ud.dir) {
			case 0:  strcpy(direction, "북"); break;
			case 1:  strcpy(direction, "북서"); break;
			case 2:  strcpy(direction, "서"); break;
			case 3:  strcpy(direction, "남서"); break;
			case 4:  strcpy(direction, "남"); break;
			case 5:  strcpy(direction, "남동"); break;
			case 6:  strcpy(direction, "동"); break;
			case 7:  strcpy(direction, "북동"); break;
			case 9:  strcpy(direction, "북"); break;
			default: strcpy(direction, "알수없음"); break;
			}
			sprintf(atcmd_output, "NPC %d: %s | 방향: %s | 스프라이트: %d | 위치: %d %d",
			        ++i, nd->name, direction, nd->class_, nd->bl.x, nd->bl.y);
			clif_displaymessage(fd, atcmd_output);
		}
		break;
	case 3:
		clif_displaymessage(fd, "----- 채팅 -----");
		iter = mapit_getallusers();
		for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
		{
			if ((cd = (struct chat_data*)map_id2bl(pl_sd->chatID)) != NULL &&
			    ...
			    cd->usersd[0] == pl_sd)
			{
				sprintf(atcmd_output, "채팅: %s | 플레이어: %s | 위치: %d %d",
				        cd->title, pl_sd->status.name, cd->bl.x, cd->bl.y);
				clif_displaymessage(fd, atcmd_output);
				sprintf(atcmd_output, "   사용자: %d/%d | 비밀번호: %s | Public: %s",
				        cd->users, cd->limit, cd->pass, (cd->pub) ? "네" : "아니오");
				clif_displaymessage(fd, atcmd_output);
			}
		}
		mapit_free(iter);
...
int atcommand_guildspy(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!enable_spy)
	{
		clif_displaymessage(fd, "맵서버에서 스파이기능 사용이 설정되지 않았습니다.");
		return -1;
	}
	if (!message || !*message || sscanf(message, "%23[^\n]", guild_name) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@길드스파이 <길드이름/ID>)");
		return -1;
	}
...
int atcommand_partyspy(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", party_name) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@파티스파이 <파티이름/ID>)");
		return -1;
	}
...
int atcommand_nuke(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@핵폭탄 <캐릭터이름/ID>)");
		return -1;
	}
...
int atcommand_tonpc(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", npcname) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@tonpc <NPC이름>)");
		return -1;
	}
...
int atcommand_shownpc(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", NPCname) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@엔피씨보이기 <NPC이름>)");
		return -1;
	}
...
int atcommand_hidenpc(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", NPCname) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@엔피씨숨기기 <NPC이름>)");
		return -1;
	}
...
int atcommand_loadnpc(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@엔피씨로드 <파일이름>)");
		return -1;
	}
...
int atcommand_unloadnpc(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%24[^\n]", NPCname) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@엔피씨언로드 <파일이름>)");
		return -1;
	}
...
int atcommand_jail(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@감옥 <캐릭터이름>)");
		return -1;
	}
...
int atcommand_unjail(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^\n]", atcmd_player_name) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@출소 <캐릭터이름>)");
		return -1;
	}
...
int atcommand_jailfor(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (year == 0 && month == 0 && day == 0 && hour == 0 && minute == 0) {
		clif_displaymessage(fd, "감금시간을 지정하지 않았습니다.");
		return -1;
	}
...
int atcommand_jailtime(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!sd->sc.data[SC_JAILED]) {
		clif_displaymessage(fd, "당신은 감옥에 있지 않습니다."); // You are not in jail.
		return -1;
	}
...
int atcommand_disguise(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@변신 <몬스터이름/ID>)");
		return -1;
	}
...
int atcommand_exp(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	sprintf(output, "레벨: %d (%.3f%%) | 직업레벨: %d (%.3f%%)", sd->status.base_level, nextb, sd->status.job_level, nextj);
...
int atcommand_broadcast(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@broadcast <글>)");
		return -1;
	}
...
int atcommand_localbroadcast(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@localbroadcast <슿>)");
		return -1;
	}
...
int atcommand_email(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%99s %99s", actual_email, new_email) < 2) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@이메일 <현재E-mail> <새로운E-mail>)");
		return -1;
	}
...
int atcommand_effect(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d", &type) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@이펙트 <번호>)");
		return -1;
	}
...
int atcommand_npcmove(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%d %d %23[^\n]", &x, &y, atcmd_player_name) < 3) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@엔피씨이동 <X> <Y> <NPC이름>)");
		return -1;
	}
	...
	{
		clif_displaymessage(fd, "NPC가 없습니다.");
	...
	clif_displaymessage(fd, "NPC를 이동 하였습니다.");
...
int atcommand_addwarp(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%31s %d %d", mapname, &x, &y) < 3) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@워프링크 <맵이름> <X> <Y>)");
		return -1;
	}
	...
	if( m == 0 )
	{
		sprintf(atcmd_output, "알수없는 맵 '%s'.", mapname);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
	...
	sprintf(atcmd_output, "새로운 워프링크 '%s' 를 만들었습니다.", nd->exname);
	clif_displaymessage(fd, atcmd_output);
	return 0;
}
...
int atcommand_follow(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		...
		clif_displaymessage(fd, "추적 모드를 해제 합니다.");
		return 0;
	}
	...
	if (sd->followtarget == pl_sd->bl.id) {
		...
		clif_displaymessage(fd, "추적 모드를 시작 합니다.");
	}
...
int atcommand_storeall(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (sd->state.storage_flag != 1)
  	{	//Open storage.
		if( storage_storageopen(sd) == 1 ) {
			clif_displaymessage(fd, "창고를 열지 못했습니다.");
			return -1;
		}
	}
	...
	clif_displaymessage(fd, "아이템을 모두 창고에 보관 하였습니다.");
	return 0;
}
...
int atcommand_skillid(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message)
	{
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@스킬아이디 <스킬이름>)");
		return -1;
	}
	...
	for (idx = 0; idx < MAX_SKILL_DB; idx++) {
		...
		{
			sprintf(atcmd_output, "스킬 %d: %s", idx, skill_db[idx].desc);
			clif_displaymessage(fd, atcmd_output);
		}
	}
...
int atcommand_useskill(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if(!message || !*message || sscanf(message, "%d %d %23[^\n]", &skillnum, &skilllv, target) != 3) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@스킬사용 <스킬번호> <스킬레벨> <사용대상>)");
		return -1;
	}
...
int atcommand_displayskill(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	{
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@스킬보기 <스킬번호> {<스킬레벨>})");
		return -1;
	}
...
int atcommand_skilltree(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if(!message || !*message || sscanf(message, "%d %23[^\r\n]", &skillnum, target) != 2) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@스킬트리 <스킬번호> <대상>)");
		return -1;
	}
	...
	sprintf(atcmd_output, "플레이어 사용 %s 스킬 트리 (%d 기본 포인트)", tbl, pc_checkskill(pl_sd, 1));
	clif_displaymessage(fd, atcmd_output);
...
int atcommand_marry(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
  ...
  if (!message || !*message || sscanf(message, "%23[^,], %23[^\r\n]", player1, player2) != 2) {
	clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@결혼 <캐릭터1>,<캐릭터2>)");
	return -1;
  }

  if((pl_sd1=map_nick2sd((char *) player1)) == NULL) {
	sprintf(player2, "'%s' 플레이어가 접속중이 아닙니다.", player1);
	clif_displaymessage(fd, player2);
	return -1;
  }

  if((pl_sd2=map_nick2sd((char *) player2)) == NULL) {
	sprintf(player1, "'%s' 플레이어가 접속중이 아닙니다.", player2);
	clif_displaymessage(fd, player1);
	return -1;
  }

  if (pc_marriage(pl_sd1, pl_sd2) == 0) {
	clif_displaymessage(fd, "지정한 두 명의 플레이어가 결혼이 성사 되었습니다.");
...
int atcommand_divorce(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
  ...
  if (!message || !*message || sscanf(message, "%23[^\r\n]", atcmd_player_name) != 1) {
	clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@이혼 <캐릭터>)");
	return -1;
  }
	...
	if (pc_divorce(pl_sd) != 0) {
		sprintf(atcmd_output, "'%s' 파트너가 없어 이혼이 실패하였습니다.", atcmd_player_name);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...
int atcommand_changelook(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if((i = sscanf(message, "%d %d", &j, &k)) < 1) {
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@체인지룩 [<위치>] <뷰코드> -- [] = 랜덤");
		clif_displaymessage(fd, "위치: 1-상단 2-중단 3-하단 4-무기 5-방패");
		return -1;
	} else if (i == 2) {
		if (j < 1) j = 1;
...
int atcommand_changegm(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	{
		clif_displaymessage(fd, "길드마스터가 사용하여야 합니다.");
		return -1;
	}

	if( map[sd->bl.m].flag.guildlock )
	{
		clif_displaymessage(fd, "현재 장소에서는 길드마스터 변경이 불가능합니다.");
		return -1;
	}

	if (strlen(message)==0)
	{
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@길마변경 <길드원이름>)");
		return -1;
	}
	
	if((pl_sd=map_nick2sd((char *) message)) == NULL || pl_sd->status.guild_id != sd->status.guild_id) {
		clif_displaymessage(fd, "해당 길드원이 접속중인 상태가 아닙니다.");
		return -1;
	}
...
int atcommand_partyoption(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	{
		clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@파티설정 <아이템공유: yes/no> <아이템분배: yes/no>)");
		return -1;
	}
...
int atcommand_autoloot(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (sd->state.autoloot) {
		snprintf(atcmd_output, sizeof atcmd_output, "몬스터가 준 아이템이 인벤토리로 들어옵니다.");
		clif_displaymessage(fd, atcmd_output);
	}else
		clif_displaymessage(fd, "몬스터가 준 아이템이 바닥에 드롭 됩니다.");
...
int atcommand_autolootitem(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		if (sd->state.autolootid) {
			...
			clif_displaymessage(fd, "명령어 사용 방법이 잘못 되었습니다. (@자동습득 <아이템이름/ID>)");
	...
	if (!item_data) {
		...
		clif_displaymessage(fd, "지정한 아이템을 찾을 수 없습니다.");
		return -1;
	}

	sd->state.autolootid = item_data->nameid; // Autoloot Activated

	sprintf(atcmd_output, "아이템 자동습득: '%s'/'%s' {%d}",
...
int atcommand_rain(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (map[sd->bl.m].flag.rain) {
		...
		clif_displaymessage(fd, "현재 지역에서 비가 그칩니다.");
	} else {
		map[sd->bl.m].flag.rain=1;
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "현재 지역에서 비가 옵니다.");
	}
	return 0;
}
...
int atcommand_snow(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (map[sd->bl.m].flag.snow) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "현재 지역에서 눈이 그칩니다.");
	} else {
		map[sd->bl.m].flag.snow=1;
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "현재 지역에서 눈이 옵니다.");
	}
...
int atcommand_sakura(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (map[sd->bl.m].flag.sakura) {
		...
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "현재 지역에서 벚꽃이 그칩니다.");
	} else {
		map[sd->bl.m].flag.sakura=1;
		clif_weather(sd->bl.m);
		clif_displaymessage(fd, "현재 지역에서 벚꽃이 내립니다.");
	}
	return 0;
}
...
{
	unsigned char msize[3][7] = {"소형", "중형", "대형"};
	unsigned char mrace[12][11] = {"무형", "불사", "동물형", "식물형", "곤충형", "어패류", "악마형", "인간형", "천사형", "용족", "보스", "일반"};
	unsigned char melement[10][8] = {"무속성", "수속성", "지속성", "화속성", "풍속성", "독속성", "성속성", "암속성", "염속성", "언데드"};
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "몬스터의 이름이나 코드를 입력해주세요 (사용법: @몹정보 <몬스터이름>).");
		return -1;
	}
	...
	for (k = 0; k < count; k++) {
		...
		if (j == 0)
			clif_displaymessage(fd, "이 몬스터는 드롭하는 아이템이 없습니다.");
		else if (j % 3 != 0)
			clif_displaymessage(fd, atcmd_output);
		// mvp
		if (mob->mexp) {
...
int atcommand_iteminfo(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "아이템의 이름이나 코드를 입력해주세요. (사용법: @아이템정보 <아이템이름>).");
		return -1;
	}
	...
	for (i = 0; i < count; i++) {
		...
		if (item_data->maxchance == 10000)
			strcpy(atcmd_output, " - 드롭율이 굉장히 높거나 상점에서 판매 중입니다");
		else if (item_data->maxchance)
			sprintf(atcmd_output, " - 가장 높은 드롭율: %02.02f%%", (float)item_data->maxchance / 100 );
		else
			strcpy(atcmd_output, " - 이 아이템은 드롭되지 않습니다");
		clif_displaymessage(fd, atcmd_output);
...
int atcommand_whodrops(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message) {
		clif_displaymessage(fd, "명령어 방식이 틀렸습니다. (예: @드롭 ).");
		return -1;
	}
	if ((item_array[0] = itemdb_exists(atoi(message))) == NULL)
		count = itemdb_searchname_array(item_array, MAX_SEARCH, message);
	...
	for (i = 0; i < count; i++) {
		...
		if (item_data->mob[0].chance == 0) {
			strcpy(atcmd_output, " - 드롭되지 않는 아이템입니다.");
			clif_displaymessage(fd, atcmd_output);
		} else {
			sprintf(atcmd_output, "- 드롭율이 높은 순서대로 정렬됩니다 (only max %d are listed):", MAX_SEARCH);
			clif_displaymessage(fd, atcmd_output);
...
int atcommand_adopt(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if (!message || !*message || sscanf(message, "%23[^,],%23[^,],%23[^\r\n]", player1, player2, player3) < 3) {
		clif_displaymessage(fd, "usage: @입양 <부>,<모>,<아이>.");
		return -1;
	}
	...
	if((pl_sd1=map_nick2sd((char *) player1)) == NULL) {
		sprintf(output, "온라인 상태의 유저 %s 를 찾을 수 없습니다.", player1);
		clif_displaymessage(fd, output);
		return -1;
	}

	if((pl_sd2=map_nick2sd((char *) player2)) == NULL) {
		sprintf(output, "온라인 상태의 유저 %s 를 찾을 수 없습니다.", player2);
		clif_displaymessage(fd, output);
		return -1;
	}
 
	if((pl_sd3=map_nick2sd((char *) player3)) == NULL) {
		sprintf(output, "온라인 상태의 유저 %s 를 찾을 수 없습니다.", player3);
		clif_displaymessage(fd, output);
		return -1;
	}
	...
	clif_displaymessage(fd, "가족이 되었습니다. 행운을 빕니다.");
	return 0;
}
...
int atcommand_rates(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	sprintf(buf, "경험치 배율: Base %.1f배 / Job %.1f배",
...
int atcommand_ksprotection(const int fd, struct map_session_data *sd, const char *command, const char *message)
{
	...
	{
		if( !message || !*message || !strcmpi(message, "파티") )
		{ // Default is Party
			sd->state.noks = 2;
...
static void atcommand_buf_sub(const int fd,struct map_session_data* sd,const int mode)
{
	if (sd && sd->state.active) 
	{
		...
		switch(mode)
		{
			...
			case 2:
				clif_skill_nodamage(&sd->bl,&sd->bl,PR_MAGNIFICAT,10,sc_start(&sd->bl,SC_MAGNIFICAT,100,10,time*1000));
				clif_skill_nodamage(&sd->bl,&sd->bl,PR_GLORIA,10,sc_start(&sd->bl,SC_GLORIA,100,10,time*1000));
			case 1:
				clif_skill_nodamage(&sd->bl,&sd->bl,HP_ASSUMPTIO,10,sc_start(&sd->bl,SC_ASSUMPTIO,100,10,time*1000));
				clif_skill_nodamage(&sd->bl,&sd->bl,PR_IMPOSITIO,10,sc_start(&sd->bl,SC_IMPOSITIO,100,10,time*1000));
			default:
				clif_skill_nodamage(&sd->bl,&sd->bl,AL_BLESSING,10,sc_start(&sd->bl,SC_BLESSING,100,10,time*1000));
				clif_skill_nodamage(&sd->bl,&sd->bl,AL_INCAGI,10,sc_start(&sd->bl,SC_INCREASEAGI,100,10,time*1000));
		}
...
int atcommand_buf(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	int mode = 0;
	struct s_mapiterator* iter;
	struct map_session_data* pl_sd;
...
/*==========================================
 * 노점검색 소스 - [JaVb] 제작, [안개] 수정
 * iLAthena 추가 부분
 *------------------------------------------*/
int atcommand_searchvend(const int fd, struct map_session_data *sd, const char *command, const char *message)
{

	struct map_session_data *pl_sd;
	struct s_mapiterator* iter;
	struct item_data *idata;
	struct item_data *c1d;
	struct item_data *c2d;
	struct item_data *c3d;
	struct item_data *c4d;
	char output[200] , item_name[100];
	char c1[100], c2[100], c3[100], c4[100];
	int cc1=0, cc2=0, cc3=0, cc4=0; 
	int refine=0, select=0;
	unsigned int hprice=0,lprice=0,count=0,j=0;
	char highchar[100]="", lowchar[100]="";
	...
	select = sscanf(message,"%99s %d %99s %99s %99s %99s",item_name, &refine, c1, c2, c3, c4);

	if (!message || !*message || select > 6) 
	{
		clif_displaymessage(fd, "올바른 아이템 이름을 입력해주세요.");
		sprintf(output,"사용법: %s <아이템 이름/ID> <제련도> <카드1> <카드2> <카드3> <카드4>",command);
		clif_displaymessage(fd, output);
		sprintf(output,"입력하신값은 아이템이름 : %s 제련도 : %d 카드1 : %s 카드2 : %s 카드3 : %s 카드4 : %s)",item_name,refine,c1,c2,c3,c4);
		clif_displaymessage(fd, output);
		return -1;
	}

	if (((idata = itemdb_searchname(item_name)) == NULL && (idata = itemdb_exists(atoi(item_name))) == NULL) ||
		((c1d = itemdb_searchname(c1)) == NULL && (c1d = itemdb_exists(atoi(c1))) == NULL && select == 3) ||
		((c2d = itemdb_searchname(c2)) == NULL && (c2d = itemdb_exists(atoi(c2))) == NULL && select == 4) ||
		((c3d = itemdb_searchname(c3)) == NULL && (c3d = itemdb_exists(atoi(c3))) == NULL && select == 5) ||
		((c4d = itemdb_searchname(c4)) == NULL && (c4d = itemdb_exists(atoi(c4))) == NULL && select == 6))
	{
		clif_displaymessage(fd, "올바른 아이템 이름을 입력해주세요.");
		sprintf(output,"사용법: %s <아이템 이름/ID> <제련도> <카드1> <카드2> <카드3> <카드4>",command);
		clif_displaymessage(fd, output);
	...
	if (select > 1 && refine < 1 || refine > 10)
	{
		clif_displaymessage(fd, "제련도 수치가 1 에서 10을 벗어났습니다.");
		sprintf(output,"사용법: %s <아이템 이름/ID> <제련도> <카드1> <카드2> <카드3> <카드4>",command);
		clif_displaymessage(fd, output);
		sprintf(output,"입력하신값은 아이템이름 : %s 제련도 : %d 카드1 : %s 카드2 : %s 카드3 : %s 카드4 : %s)",item_name,refine,c1,c2,c3,c4);
		clif_displaymessage(fd, output);
	...
	// Header 
	sprintf(output,"* [ 요청하신 아이템 %s의 검색 결과 ] *",idata->jname);
	clif_displaymessage(fd,output);

	iter = mapit_getallusers();
	for( pl_sd = (TBL_PC*)mapit_first(iter); mapit_exists(iter); pl_sd = (TBL_PC*)mapit_next(iter) )
	{
		if ((pl_sd)&&(pl_sd->status.account_id!=sd->status.account_id)&&(pl_sd->vender_id))
		{
			for(j = 0; j<pl_sd->vend_num; j++)
			{
				...
				case 1:
					{
						if(pl_sd->status.cart[pl_sd->vending[j].index].nameid==idata->nameid)
						{
							if(count==0)
							{
								lprice=pl_sd->vending[j].value;
								strcpy(lowchar,pl_sd->status.name);
								hprice=pl_sd->vending[j].value;
								strcpy(highchar,(pl_sd->status.name));
							}
							else
							{
								if (hprice<pl_sd->vending[j].value)
								{
									hprice=pl_sd->vending[j].value;
									strcpy(highchar,(pl_sd->status.name));
								}
								if (lprice>pl_sd->vending[j].value)
								{
									lprice=pl_sd->vending[j].value;
									strcpy(lowchar,pl_sd->status.name);
								}
				...
				case 2:
					{
						if(pl_sd->status.cart[pl_sd->vending[j].index].nameid == idata->nameid &&
							pl_sd->status.cart[pl_sd->vending[j].index].refine == refine)
						{
							if(count==0)
							{
								lprice=pl_sd->vending[j].value;
				...
				case 3:
					{
						if(pl_sd->status.cart[pl_sd->vending[j].index].nameid == idata->nameid &&
							pl_sd->status.cart[pl_sd->vending[j].index].refine == refine &&
							pl_sd->status.cart[pl_sd->vending[j].index].card[0] == c1d->nameid)
						{
							if(count==0)
							{
								lprice=pl_sd->vending[j].value;
				...
				case 4:
					{
						if(pl_sd->status.cart[pl_sd->vending[j].index].nameid == idata->nameid &&
							pl_sd->status.cart[pl_sd->vending[j].index].refine == refine &&
							pl_sd->status.cart[pl_sd->vending[j].index].card[0] == c1d->nameid &&
							pl_sd->status.cart[pl_sd->vending[j].index].card[1] == c2d->nameid)
						{
							if(count==0)
							{
								lprice=pl_sd->vending[j].value;
				...
				case 5:
					{
						if(pl_sd->status.cart[pl_sd->vending[j].index].nameid == idata->nameid &&
							...
							pl_sd->status.cart[pl_sd->vending[j].index].card[0] == c1d->nameid &&
							pl_sd->status.cart[pl_sd->vending[j].index].card[1] == c2d->nameid &&
							pl_sd->status.cart[pl_sd->vending[j].index].card[2] == c3d->nameid)
						{
							if(count==0)
							{
								lprice=pl_sd->vending[j].value;
				...
				case 6 :
					{
						if(pl_sd->status.cart[pl_sd->vending[j].index].nameid == idata->nameid &&
							...
							pl_sd->status.cart[pl_sd->vending[j].index].card[1] == c2d->nameid &&
							pl_sd->status.cart[pl_sd->vending[j].index].card[2] == c3d->nameid &&
							pl_sd->status.cart[pl_sd->vending[j].index].card[3] == c4d->nameid)
						{
							if(count==0)
							{
								lprice=pl_sd->vending[j].value;
					...
					if (pl_sd->status.cart[pl_sd->vending[j].index].nameid == idata->nameid)
					{
						sprintf(output,"- [이 름]: %s , [위치]: M=%s X=%d Y=%d , [판매 가격]: %d 제니 , [남은 갯수]: %d 개",pl_sd->status.name,mapindex_id2name(pl_sd->mapindex),pl_sd->bl.x,pl_sd->bl.y,pl_sd->vending[j].value,pl_sd->vending[j].amount);
						clif_displaymessage(fd,output);
						count++;
					}
				...
				if(count>=MAX_SEARCH) break;
					...
					if(count==0) { 
			clif_displaymessage(fd,"+ 현재 판매중인 상인이 없습니다. +");
	}
	if(count>0) {	
		sprintf(output,"+ [최대가격] - %d 제니 (판매자: %s) +",hprice,highchar);
		clif_displaymessage(fd,output);
		sprintf(output,"+ [최소가격] - %d 제니 (판매자: %s) +",lprice,lowchar);
		clif_displaymessage(fd,output);
	}
...
/*==========================================
 * 랜덤 파티 등록 - [★orz。] 제작, [건신] 아이디어.
 * iLAthena 추가 부분
 *------------------------------------------*/
int Timer_RandomParty_Invite(int tid, unsigned int tick, int id, intptr data)
{
	struct map_session_data* sd = map_id2sd(id);
	struct map_session_data *pl_sd;
	struct s_mapiterator* iter;
	int count=0;
	const unsigned int SHARE_BLV=10;	// inter_config::party_share_level 에서설정(예정)한균등파티레벨제한
	const unsigned int PARTY_MAX_LEVEL=sd->status.base_level+(SHARE_BLV/2);
	const unsigned int PARTY_MIN_LEVEL=sd->status.base_level-(SHARE_BLV/2);
	...
	{
		// 자기자신일경우continue
		if(pl_sd->bl.id == sd->bl.id)
			continue;
		// 랜덤파티등록이안되있거나, 파티가존재하는지, 초대중인지에따른필터링
		if(!pl_sd->status.party_entry || pl_sd->party_invite || pl_sd->status.party_id)
			continue;
		// 기본기능체크 ; 1.03 추가 시작
		if( pc_checkskill(pl_sd, 1) < 5 )
			continue;
// 1.03 추가 종료
		// 서버에서설정한inter_config::party_share_level에따른(예정) 필터링
		if(pl_sd->status.base_level <= PARTY_MAX_LEVEL && pl_sd->status.base_level >= PARTY_MIN_LEVEL)	// 1.03 수정
		{
			party_invite(sd, pl_sd);
			count++;
		}
	}
	...
	if (count == 0)
		clif_displaymessage(sd->fd, msg_txt(28)); // No player found.
	else if (count == 1)
		clif_displaymessage(sd->fd, msg_txt(29)); // 1 player found.
	else {
		sprintf(atcmd_output, msg_txt(30), count); // %d players found.
		clif_displaymessage(sd->fd, atcmd_output);
	}
...
int atcommand_RandomParty_Entry(const int fd, struct map_session_data *sd, const char *command, const char *message)
{
	nullpo_retr(-1,sd);

	// 기본기능체크 ; 1.03 추가 시작
	if( pc_checkskill(sd, 1) < 5 )
	{
		clif_displaymessage(fd, "기본기능 스킬의 레벨이 부족합니다.");
		return -1;
	}
// 1.03 추가 종료
	if(sd->party_invite || sd->status.party_id) 
	{
		clif_displaymessage(fd, "이미 파티에 가입되어 있습니다. 설정을 변경할 수 없습니다.");
		return -1;
	} 
	else if( sd->status.party_entry ) 
	{
		sd->status.party_entry = 0;
		clif_displaymessage(fd, "랜덤파티 등록이 해제되었습니다.");
	} 
	else
	{
		sd->status.party_entry = 1;
		clif_displaymessage(fd, "랜덤파티 등록이 설정되었습니다.");
	}

	return 0;
}
int atcommand_RandomParty_Create(const int fd, struct map_session_data *sd, const char *command, const char *message)
{
	char party_name[NAME_LENGTH];
	char tparty_name[NAME_LENGTH];
...
// 1.03 추가 종료
	...
	if(sd->party_invite) 
	{
		clif_displaymessage(fd, "파티에 초대받고 있습니다.");
		return -1;
	}
	...
	if ((!message || !*message)) {
		if ((p && !p->party.member[i].leader) || !p)
		{
			clif_displaymessage(fd,"사용법: @랜파생성 <파티이름>");
			return -1;
		}
	}

	if(sd->status.party_entry)
	{
		if (p)
		{
			...
			clif_displaymessage(fd, "이미 파티에 가입되어 있습니다. 대신 파티원만 초대합니다.");
		}
		else
		{
			sprintf(tparty_name, "[랜파] %s♡", message);
			safestrncpy(party_name, tparty_name, NAME_LENGTH);
			party_create(sd, party_name, 0, 0);
		}
		clif_displaymessage(fd, "5초뒤 랜덤파티에 등록된 파티원을 초대합니다.");
		add_timer(gettick()+5000, Timer_RandomParty_Invite, sd->bl.id, 0);
	}
	else
	{
		clif_displaymessage(fd, "랜덤파티 등록을 먼저해주십시오.");
		return -1;
	}
	
	return 0;
}
int atcommand_RandomParty_Check(const int fd, struct map_session_data *sd, const char *command, const char *message)
{
	char temp0[100];
	struct map_session_data *pl_sd;
	struct s_mapiterator* iter;
 ...
 * iLAthena 추가 부분
 *------------------------------------------*/
int atcommand_lockdown(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	int flag=3;
	...
	if (!message || !*message) {
		clif_displaymessage(sd->fd,"[lockdown]: 해당 유저의 이동,스킬,아이템사용을 차단하거나 허용합니다.");
		clif_displaymessage(sd->fd,"               <사용법: @lockdown (플래그) (캐릭터명) >");
		clif_displaymessage(sd->fd,"* 플래그 :  0-움직임 잠금  ,  1-움직임 허용    ,  2-움직임 잠금 확인 ");
		clif_displaymessage(sd->fd,"            3-스킬   잠금  ,  4-스킬   허용    ,  5-스킬   잠금 확인 ");
		clif_displaymessage(sd->fd,"            6-아이템 잠금  ,  7-아이템 허용    ,  8-아이템 잠금 확인 ");
		clif_displaymessage(sd->fd,"           10-이동+스킬잠금, 11-이동+아이템잠금,  12-스킬+아이템잠금 ");
		clif_displaymessage(sd->fd," 20-이동+스킬잠금해체, 21-이동+아이템잠금해체, 22-스킬+아이템잠금해체");
		clif_displaymessage(sd->fd," 90-전체 잠금 , 91-전체 해체");
		return -1;
	}
	...
	target_sd=map_nick2sd(player);
	
	if(!target_sd) {
		clif_displaymessage(sd->fd,"[lockdown]: 해당 유저가 존재하지 않습니다.");
		return -1;
	}
	
	if(pc_isGM(sd)<pc_isGM(target_sd)) {
		clif_displaymessage(sd->fd,"[lockdown]: 권한이 없습니다.");
		return -1;
	}
	...
	{
		case 0:
			if(target_sd->state.blockedmove==0) {
				target_sd->state.blockedmove=1;
				clif_displaymessage(sd->fd,"[lockdown]: 해당 유저의 움직임을 차단하였습니다.");
				return 0;
			}
			else {
				clif_displaymessage(sd->fd,"[lockdown]: 이미 움직임이 차단되어 있는 유저입니다.");
				return -1;
			}
			
		case 1:
			if(target_sd->state.blockedmove!=0) {
				target_sd->state.blockedmove=0;
				clif_displaymessage(sd->fd,"[lockdown]: 해당 유저의 움직임을 허용하였습니다.");
				return 0;
			}
			else {
				clif_displaymessage(sd->fd,"[lockdown]: 이미 움직임이 허용되어 있는 유저입니다.");
				return -1;
			}
			
		case 2:
			if(target_sd->state.blockedmove==0) {
				clif_displaymessage(sd->fd,"[lockdown]: 해당 유저는 현재 움직임이 허용된 상태입니다.");
				return 0;
			}
			else {
				clif_displaymessage(sd->fd,"[lockdown]: 해당 유저는 현재 움직임이 차단된 상태입니다.");
				return 0;
			}

		case 3:

			if(target_sd->state.blockedallskill==0) {
				target_sd->state.blockedallskill=1;
				clif_displaymessage(sd->fd,"[lockdown]: 해당 유저의 스킬사용을 차단하였습니다.");
				return 0;
			}
		case 4:
			if(target_sd->state.blockedallskill!=0) {
				target_sd->state.blockedallskill=0;
				clif_displaymessage(sd->fd,"[lockdown]: 해당 유저의 스킬사용을 허용하였습니다.");
				return 0;
			}
		case 5:
			if(target_sd->state.blockedallskill==0) {
				clif_displaymessage(sd->fd,"[lockdown]: 해당 유저는 현재 스킬사용이 허용된 상태입니다.");
				return 0;
			}
			else {
				clif_displaymessage(sd->fd,"[lockdown]: 해당 유저는 현재 스킬사용이 차단된 상태입니다.");
				return 0;
			}
		case 6:
			if(target_sd->state.blockeduseitem==0) {
				target_sd->state.blockeduseitem=1;
				clif_displaymessage(sd->fd,"[lockdown]: 해당 유저의 아이템사용을 차단하였습니다.");
				return 0;
			}
		case 7:
			if(target_sd->state.blockeduseitem!=0) {
				target_sd->state.blockeduseitem=0;
				clif_displaymessage(sd->fd,"[lockdown]: 해당 유저의 아이템사용을 허용하였습니다.");
				return 0;
			}
		case 8:
			if(target_sd->state.blockeduseitem==0) {
				clif_displaymessage(sd->fd,"[lockdown]: 해당 유저는 현재 아이템사용이 허용된 상태입니다.");
				return 0;
			}
			else {
				clif_displaymessage(sd->fd,"[lockdown]: 해당 유저는 현재 아이템사용이 차단된 상태입니다.");
				return 0;
			}
		case 10:
			strcpy(output,"[lockdown]: 이동-<");
			
			if(target_sd->state.blockedmove==0) {
				target_sd->state.blockedmove=1;
				strcat(output,"잠금 완료> 스킬-<");
			} else {
				strcat(output,"이미 잠김> 스킬-<");
			}
		...
		case 11:
			...
			if(target_sd->state.blockedmove==0) {
				target_sd->state.blockedmove=1;
				strcat(output,"잠금 완료> 아이템-<");
			} else {
				strcat(output,"이미 잠김> 아이템-<");
			}
		...
		case 20:
			...
			if(target_sd->state.blockedmove!=0) {
				target_sd->state.blockedmove=0;
				strcat(output,"해체 완료> 스킬-<");
			} else {
				strcat(output,"이미 해체> 스킬-<");
			}
		...
		case 21:
			...
			if(target_sd->state.blockedmove!=0) {
				target_sd->state.blockedmove=0;
				strcat(output,"해체 완료> 아이템-<");
			} else {
				strcat(output,"이미 해체> 아이템-<");
			}
		...
		default:
			clif_displaymessage(sd->fd,"[lockdown]: 알수없는 플래그 입니다. 확인후 재시도 해주십시오.");
			clif_displaymessage(sd->fd,"            <사용법: @lockdown (플래그) (캐릭터명) >");
			clif_displaymessage(sd->fd,"* 플래그 :  0-움직임 잠금  ,  1-움직임 허용    ,  2-움직임 잠금 확인 ");
			clif_displaymessage(sd->fd,"            3-스킬   잠금  ,  4-스킬   허용    ,  5-스킬   잠금 확인 ");
			clif_displaymessage(sd->fd,"            6-아이템 잠금  ,  7-아이템 허용    ,  8-아이템 잠금 확인 ");
...
/*==========================================
 * 특정맵에 있는 유저들에게 아이템 주기 - 추가시작
 *------------------------------------------*/
int atcommand_itemmap(
    const int fd, struct map_session_data* sd, 
    const char* command, const char* message )
{
    struct map_session_data *pl_sd = NULL;
    struct map_session_data **pl_allsd = NULL;
    struct s_mapiterator* iter = NULL;
    struct item item_tmp;
    struct item_data* item_data = NULL;
    char map_name[MAP_NAME_LENGTH] = {0};
    char item_name[100] = {0};
    int i=0,j=0, users=0;
    int pl_GM_level=0, GM_level=0;
    int map_id = 0;
    ...
    if( !message || !*message || 
        ...
        sscanf( message, "%11s \"%99[^\"]\" %d", map_name, item_name, &number ) != 3 &&
        sscanf( message, "%11s %99s %d", map_name, item_name, &number) != 3) {
            clif_displaymessage( fd, "명령어 구문사용 오류 (@맵아이템 <맵> <아이템> [수])" );
        return 0;
     }
...
if(!itemdb_isstackable2(item_data))
    ...
    if(strstr( map_name, ".gat") == NULL && strstr( map_name, ".afm") == NULL && strlen(map_name) < MAP_NAME_LENGTH-4) 
		strcat(map_name, ".gat");
    if((map_id = map_mapname2mapid(map_name)) < 0)
        map_id = sd->bl.m;
...
//Logs (A)dmins items
				...
				clif_displaymessage( pl_sd->fd, "운영자로부터 아이템이 지급되었습니다." );
				}
		}
	}
	mapit_free( iter );

	clif_displaymessage(fd, "플레이어에게 아이템이 지급되었습니다." );
...
char item_name[100] = {0};
    ...
    if( !message || !*message || 
		...
		sscanf( message, "%99[^\"]\" %d", item_name, &number ) != 2 &&
		sscanf( message, "%99s %d", item_name, &number) != 2) {
		clif_displaymessage(fd, "명령어 구문사용 오류 (@아이템지급 <아이템> [수])" );
        return 0;
     }
...
// Iterate through all players in map server
...
// You can only look lower or same level
        if( !( ( battle_config.hide_GM_session || ( pl_sd->sc.option & OPTION_INVISIBLE ) ) && ( pl_GM_level > GM_level ) ) )
			{
				for( j = 0; j < number; j += get_count )
				{
...
//Logs (A)dmins items
				...
				clif_displaymessage( pl_sd->fd, "운영자가 아이템을 지급하였습니다." );
				}
 ...
 * 이벤트 명령어 [JaVb]
 *------------------------------------------*/
int eevent_timer(int tid, unsigned int tick, int id, int data)
{
	char output[200];
		...
		case 1:
			if(event_eflag==1&&event_dflag==0) {
				...
				if(event_tick<=0) {
					...
					sprintf(output,"[ 경험치 이벤트가 %d분 남았습니다. ]",event_tick);
					intif_broadcast(output,strlen(output)+1,0);
					add_timer(gettick()+60000,eevent_timer,event_tid,1);
				}
				return 1;
			}
			return 1;
		...
		case 2:
			if(event_dflag==1&&event_eflag==0) {
				...
				if(event_tick<=0) {
					...
					sprintf(output,"[ 드롭률 이벤트가 %d분 남았습니다. ]",event_tick);
					intif_broadcast(output,strlen(output)+1,0);
					add_timer(gettick()+60000,eevent_timer,event_tid,2);
				}
				return 1;
			}
			return 1;
		case 3:
			if(event_eflag==1&&event_dflag==1) {
				...
				if(event_tick<=0) {
					...
					sprintf(output,"[ 경험치획득량과 아이템드롭률 이벤트가 %d분 남았습니다. ]",event_tick);
					intif_broadcast(output,strlen(output)+1,0);
					add_timer(gettick()+60000,eevent_timer,event_tid,3);
					return 1;
				}
			}
			return 1;
...
int atcommand_eevent(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	char output[200];
	
	int spl,flag=0,rate,tick,timer;
	double i=0;

	if(!message||!*message) {
		clif_displaymessage(fd,"[이벤트]: 사용방법을 확인해 주십시오.");
		sprintf(output,"<사용법: %s Tick(Min) Rate(10:1배) Flag1(1:2:3) Flag2(0(Default):1:2:3) >",command);
		clif_displaymessage(fd,output);
		return -1;
	}
	...
	// 각 입력된 데이터 확인
	if(flag<0||flag>3) {
		clif_displaymessage(fd,"[이벤트]: 플래그를 확인해 주십시오. ");
		sprintf(output,"<사용법: %s Tick(Min) Rate(10:1배) Flag1(1:2:3) Flag2(0(Default):1:2:3) >",command);
		clif_displaymessage(fd,output);
		return -1;
	}
	...
	switch(spl) {
		case 1: // 경험치
			...
			case 1:
				...
				// 본격적인 Timer 설정 시작
				timer=add_timer(gettick()+60000,eevent_timer,event_tid,1);
			...
			case 2:
				...
				// 먼저 Timer 삭제
				delete_timer(event_tid,eevent_timer);
				...
				// 끝... 이제 방송
				intif_broadcast("[ 경험치 이벤트가 종료되었습니다. ]",strlen("[ 경험치 이벤트가 종료되었습니다. ]")+1,0);
		...
		case 2:
			...
			case 1:
				...
				if(event_rate!=rate) {
					...
					// 본격적인 Timer 설정 시작
				timer=add_timer(gettick()+60000,eevent_timer,event_tid,2);
				event_tid=timer;
			...
			case 2:
				...
				// 끝... 이제 방송
				intif_broadcast("[ 드롭률 이벤트가 종료되었습니다. ]",strlen("[ 드롭률 이벤트가 종료되었습니다. ]")+1,0);
		...
		case 3: // 통합
			...
			case 1:
				...
				timer=add_timer(gettick()+60000,eevent_timer,event_tid,3);
			...
			case 2:
				...
				// 끝... 이제 방송
				intif_broadcast("[ 통합 이벤트가 종료되었습니다. ]",strlen("[ 통합 이벤트가 종료되었습니다. ]")+1,0);
...
int atcommand_charkami(const int fd, struct map_session_data *sd, const char *command, const char *message)
{
	...
	if(!message || !*message) {
		sprintf(output,"메시지와 컬러값을 입력해주세요. (사용법: %s <색코드(RRGGBB)> <메시지>)",command);
		clif_displaymessage(fd,output);
		return -1;
	}

	if(sscanf(message, "%lx %199[^\n]", &color, atcmd_output) < 1) {
		if (!atcmd_output || !*atcmd_output) {
			clif_displaymessage(fd,"메시지를 입력해 주십시오.");
			return -1;
		}
	...
	// 레벨제한채크
	if(battle_config.charkami_grantlevel>sd->status.base_level) {
		sprintf(output,"사용가능한 레벨은 %d이상부터입니다.",sd->status.base_level);
		clif_displaymessage(fd,output);
		return -1;
	}

	// 돈사용여부채크
	if(battle_config.charkami_usemoney>sd->status.zeny) {
		sprintf(output,"소지금이 부족합니다. 1회 사용료는 %d 입니다.",sd->status.zeny);
		clif_displaymessage(fd,output);
		return -1;
	}

	// 아이템사용여부채크
	if(battle_config.charkami_useitem==1) {
		// 아이템사용이니아이템코드채크
		if((idata=itemdb_exists(battle_config.charkami_useitemcode))==NULL) {
			ShowError("[CharKami]: Unknown item code. ( set: %d )\n",battle_config.charkami_useitemcode);
			battle_config.charkami_useitem=0;
		}
		
		// 아이템소지여부채크
		if(battle_config.charkami_useitem==1) {
			litem = pc_search_inventory(sd,battle_config.charkami_useitemcode);

			if(litem<0) {
				sprintf(output,"필요아이템이 없습니다. 기능사용을 위해서는 %d개의 %s가 필요합니다.",battle_config.charkami_useitemamount,idata->jname);
				clif_displaymessage(fd,output);
				return -1;
			}

			if(sd->status.inventory[litem].amount<battle_config.charkami_useitemamount) {
				sprintf(output,"필요아이템이 필요수량보다 적습니다. 기능사용을 위해서는 %d개의 %s가 필요합니다.",battle_config.charkami_useitemamount,idata->jname);
				clif_displaymessage(fd,output);
				return -1;
			}
	...
	// 아이템
	if(battle_config.charkami_useitem==1) {
		if(pc_delitem(sd,litem,battle_config.charkami_useitemamount,0)) {
			clif_displaymessage(fd,"필요아이템이 없습니다.");
			return -1;
		}
	}
	...
	// 제니삭제
	if((battle_config.charkami_usemoney>0)) {
		if(pc_payzeny(sd,battle_config.charkami_usemoney)) {
			clif_displaymessage(fd,"소지금이 부족합니다.");
			return -1;
		}
	}

	// 정상.
	sprintf(output,"[%s]: %s",sd->status.name,atcmd_output);
	intif_broadcast(output, strlen(output) + 1, color);

	return 0;
}

/*==========================================
 * atcommand_info[] 구조 정의
 *------------------------------------------*/

AtCommandInfo atcommand_info[] = {
	{ "워프",              40,40,     atcommand_mapmove },
	{ "이동",              40,40,     atcommand_mapmove },
	{ "맵",           40,40,     atcommand_mapmove }, // + /mm
	{ "어디",              1,1,      atcommand_where },
	{ "출두",            20,20,     atcommand_jumpto }, // + /shift
	{ "플레이어",            20,20,     atcommand_jumpto },
	{ "점프투",              20,20,     atcommand_jumpto },
	{ "점프",              40,40,     atcommand_jump },
	{ "누구",               20,20,     atcommand_who },
	{ "접속자",             20,20,     atcommand_who },
	{ "누구2",              20,20,     atcommand_who2 },
	{ "누구3",              20,20,     atcommand_who3 },
	{ "누구맵",            20,20,     atcommand_whomap },
	{ "누구맵2",           20,20,     atcommand_whomap2 },
	{ "누구맵3",           20,20,     atcommand_whomap3 },
	{ "운영자보기",             20,20,     atcommand_whogm },
	{ "저장",              40,40,     atcommand_save },
	{ "로드",            40,40,     atcommand_load },
	{ "집",              40,40,     atcommand_load },
	{ "속도",             40,40,     atcommand_speed },
	{ "창고",            1,1,      atcommand_storage },
	{ "길드창고",          50,50,     atcommand_guildstorage },
	{ "옵션",            40,40,     atcommand_option },
	{ "숨기",              40,40,     atcommand_hide }, // + /hide
	{ "직업",         40,40,     atcommand_jobchange },
	{ "전직",               40,40,     atcommand_jobchange },
	{ "사망",                1,1,      atcommand_die },
	{ "킬",              60,60,     atcommand_kill },
	{ "부활",             60,60,     atcommand_alive },
	{ "공지",              40,40,     atcommand_kami },
	{ "파란공지",             40,40,     atcommand_kami },
	{ "색공지",             40,40,     atcommand_kami },
	{ "힐",              40,60,     atcommand_heal },
	{ "아이템",              60,60,     atcommand_item },
	{ "아이템2",             60,60,     atcommand_item2 },
	{ "인벤비우기",         40,40,     atcommand_itemreset },
	{ "레벨",              60,60,     atcommand_baselevelup },
	{ "베렙",              60,60,     atcommand_baselevelup },
	{ "렙",            60,60,     atcommand_baselevelup },
	{ "베이스",           60,60,     atcommand_baselevelup },
	{ "베이스레벨",          60,60,     atcommand_baselevelup },
	{ "베이스렙",         60,60,     atcommand_baselevelup },
	{ "렙업",         60,60,     atcommand_baselevelup },
	{ "잡업",              60,60,     atcommand_joblevelup },
	{ "잡레벨",            60,60,     atcommand_joblevelup },
	{ "잡렙",            60,60,     atcommand_joblevelup },
	{ "직업레벨",          60,60,     atcommand_joblevelup },
	{ "잡렙업",           60,60,     atcommand_joblevelup },
	{ "잡옵",          60,60,     atcommand_joblevelup },
	{ "도움말",                 20,20,     atcommand_help },
	{ "도움",              20,20,     atcommand_help },
	{ "도움말2",                20,20,     atcommand_help2 },
	{ "도움2",             20,20,     atcommand_help2 },
	{ "피케이끄기",            40,40,     atcommand_pvpoff },
	{ "피케이",             40,40,     atcommand_pvpon },
	{ "길드전종료",            40,40,     atcommand_gvgoff },
	{ "길드전끄기",           40,40,     atcommand_gvgoff },
	{ "길드전",             40,40,     atcommand_gvgon },
	{ "길드전시작",            40,40,     atcommand_gvgon },
	{ "모델",             20,20,     atcommand_model },
	{ "마을",                10,10,     atcommand_go },
	{ "몬스터",           50,50,     atcommand_monster },
	{ "몹",             50,50,     atcommand_monster },
	{ "작은몹",      50,50,     atcommand_monstersmall },
	{ "큰몹",        50,50,     atcommand_monsterbig },
	{ "킬몹",       60,60,     atcommand_killmonster },
	{ "킬몹2",      40,40,     atcommand_killmonster2 },
	{ "제련",            60,60,     atcommand_refine },
	{ "제조",           60,60,     atcommand_produce },
	{ "메모",              40,40,     atcommand_memo },
	{ "gat",               99,99,     atcommand_gat },
	{ "displaystatus",     99,99,     atcommand_displaystatus },
	{ "스텟포인트",           60,60,     atcommand_statuspoint },
	{ "스킬포인트",           60,60,     atcommand_skillpoint },
	{ "제니",              60,60,     atcommand_zeny },
	{ "힘",               60,60,     atcommand_param },
	{ "어",               60,60,     atcommand_param },
	{ "바",               60,60,     atcommand_param },
	{ "인",               60,60,     atcommand_param },
	{ "덱",               60,60,     atcommand_param },
	{ "럭",               60,60,     atcommand_param },
	{ "길드레벨",              60,60,     atcommand_guildlevelup },
	{ "길렙",            60,60,     atcommand_guildlevelup },
	{ "길렙업",          60,60,     atcommand_guildlevelup },
	{ "길업",         60,60,     atcommand_guildlevelup },
	{ "길드레벨업",        60,60,     atcommand_guildlevelup },
	{ "길드렙업",        60,60,     atcommand_guildlevelup },
	{ "펫생성",           60,60,     atcommand_makeegg },
	{ "펫부화",             60,60,     atcommand_hatch },
	{ "펫친밀도",       40,40,     atcommand_petfriendly },
	{ "펫만복도",         40,40,     atcommand_pethungry },
	{ "펫이름",          1,1,      atcommand_petrename },
	{ "소환",            60,60,     atcommand_recall }, // + /recall
	{ "밤",             80,80,     atcommand_night },
	{ "낮",               80,80,     atcommand_day },
	{ "심판",              80,80,     atcommand_doom },
	{ "맵심판",           80,80,     atcommand_doommap },
	{ "축복",             80,80,     atcommand_raise },
	{ "맵축복",          80,80,     atcommand_raisemap },
	{ "킥",              20,20,     atcommand_kick }, // + right click menu for GM "(name) force to quit"
	{ "올킥",           99,99,     atcommand_kickall },
	{ "올스킬",          60,60,     atcommand_allskill },
	{ "전체스킬",         60,60,     atcommand_allskill },
	{ "스킬전부",          60,60,     atcommand_allskill },
	{ "allskill",         60,60,     atcommand_allskill },
	{ "퀘스킬",        40,40,     atcommand_questskill },
	{ "퀘스킬2",         40,40,     atcommand_lostskill },
	{ "축기",        40,40,     atcommand_spiritball },
	{ "파티",              1,1,      atcommand_party },
	{ "길드",             50,50,     atcommand_guild },
	{ "공성시작",         60,60,     atcommand_agitstart },
	{ "공성종료",           60,60,     atcommand_agitend },
	{ "맵다운",           99,99,     atcommand_mapexit },
	{ "아이템검색",          60,60,     atcommand_idsearch },
	{ "broadcast",         40,40,     atcommand_broadcast }, // + /b and /nb
	{ "localbroadcast",    40,40,     atcommand_localbroadcast }, // + /lb and /nlb
	{ "전체소환",         80,80,     atcommand_recallall },
	{ "아이템디비",      99,99,     atcommand_reloaditemdb },
	{ "몬스터디비",       99,99,     atcommand_reloadmobdb },
	{ "스킬디비",     99,99,     atcommand_reloadskilldb },
	{ "스크립트",      99,99,     atcommand_reloadscript },
	{ "운영자디비",   99,99,     atcommand_reloadatcommand },
	{ "배틀디비",  99,99,     atcommand_reloadbattleconf },
	{ "상태디비",    99,99,     atcommand_reloadstatusdb },
	{ "피씨디비",        99,99,     atcommand_reloadpcdb },
	{ "공지디비",        99,99,     atcommand_reloadmotd },
	{ "맵정보",           99,99,     atcommand_mapinfo },
	{ "옷염색",               40,40,     atcommand_dye },
	{ "옷색",            40,40,     atcommand_dye },
	{ "머리모양",         40,40,     atcommand_hair_style },
	{ "헤어스타일",            40,40,     atcommand_hair_style },
	{ "머리색",         40,40,     atcommand_hair_color },
	{ "머리염색",            40,40,     atcommand_hair_color },
	{ "올스텟",           60,60,     atcommand_stat_all },
	{ "올스테이스터스",          60,60,     atcommand_stat_all },
	{ "모든스텟",          60,60,     atcommand_stat_all },
	{ "스텟전부",           60,60,     atcommand_stat_all },
	{ "캐릭블럭",             60,60,     atcommand_char_block },
	{ "캐릭터블럭",         60,60,     atcommand_char_block },
	{ "캐릭터밴",               60,60,     atcommand_char_ban },
	{ "캐릭밴",            60,60,     atcommand_char_ban },
	{ "캐릭터차단",           60,60,     atcommand_char_ban },
	{ "캐릭차단",        60,60,     atcommand_char_ban },
	{ "캐릭블럭해제",           60,60,     atcommand_char_unblock },
	{ "캐릭터블럭해제",       60,60,     atcommand_char_unblock },
	{ "캐릭밴해제",             60,60,     atcommand_char_unban },
	{ "캐릭터밴해제",          60,60,     atcommand_char_unban },
	{ "캐릭차단해제",         60,60,     atcommand_char_unban },
	{ "캐릭터차단해제",      60,60,     atcommand_char_unban },
	{ "페코",             20,20,     atcommand_mount_peco },
	{ "페코타기",         20,20,     atcommand_mount_peco },
	{ "길드스파이",          60,60,     atcommand_guildspy },
	{ "파티스파이",          60,60,     atcommand_partyspy },
	{ "전체수리",         60,60,     atcommand_repairall },
	{ "길드소환",       60,60,     atcommand_guildrecall },
	{ "파티소환",       60,60,     atcommand_partyrecall },
	{ "핵폭탄",              60,60,     atcommand_nuke },
	{ "엔피씨보이기",           80,80,     atcommand_shownpc },
	{ "엔피씨숨기기",           80,80,     atcommand_hidenpc },
	{ "엔피씨로드",           80,80,     atcommand_loadnpc },
	{ "엔피씨언로드",         80,80,     atcommand_unloadnpc },
	{ "시간",               1,1,      atcommand_servertime },
	{ "날짜",               1,1,      atcommand_servertime },
	{ "서버시간",         1,1,      atcommand_servertime },
	{ "서버날짜",         1,1,      atcommand_servertime },
	{ "감옥",              60,60,     atcommand_jail },
	{ "감옥해제",            60,60,     atcommand_unjail },
	{ "출소",         60,60,     atcommand_unjail },
	{ "감금",           60,60,     atcommand_jailfor },
	{ "감옥시간",           1,1,      atcommand_jailtime },
	{ "변신",          20,20,     atcommand_disguise },
	{ "변신해제",        20,20,     atcommand_undisguise },
	{ "이메일",              1,1,      atcommand_email },
	{ "이펙트",            40,40,     atcommand_effect },
	{ "추적",            20,20,     atcommand_follow },
	{ "워프링크",           60,60,     atcommand_addwarp },
	{ "스킬사용",           80,80,     atcommand_skillon },
	{ "스킬금지",          80,80,     atcommand_skilloff },
	{ "킬러",            60,60,     atcommand_killer },
	{ "엔피씨이동",           80,80,     atcommand_npcmove },
	{ "무방비",          40,40,     atcommand_killable },
	{ "전부드랍",           40,40,     atcommand_dropall },
	{ "창고보관",          40,40,     atcommand_storeall },
	{ "스킬아이디",           40,40,     atcommand_skillid },
	{ "스킬사용",          40,40,     atcommand_useskill },
	{ "스킬보기",      99,99,     atcommand_displayskill },
	{ "눈",              99,99,     atcommand_snow },
	{ "벚꽃",            99,99,     atcommand_sakura },
	{ "구름",            99,99,     atcommand_clouds },
	{ "구름2",           99,99,     atcommand_clouds2 },
	{ "안개",               99,99,     atcommand_fog },
	{ "폭죽",         99,99,     atcommand_fireworks },
	{ "낙엽",            99,99,     atcommand_leaves },
	{ "용병",            60,60,     atcommand_summon },
	{ "임시운영자",          99,99,     atcommand_adjgmlvl },
	{ "명령어권한",         99,99,     atcommand_adjcmdlvl },
	{ "거래",             60,60,     atcommand_trade },
	{ "패킷전송",              99,99,     atcommand_send },
	{ "배틀플래그설정",     99,99,     atcommand_setbattleflag },
	{ "채금해제",            80,80,     atcommand_unmute },
	{ "날씨해제",      99,99,     atcommand_clearweather },
	{ "업타임",             1,1,      atcommand_uptime },
	{ "성전환",         60,60,     atcommand_changesex },
	{ "채금",              80,80,     atcommand_mute },
	{ "새로고침",            1,1,      atcommand_refresh },
	{ "감정",          40,40,     atcommand_identify },
	{ "공지보기",             20,20,     atcommand_gmotd },
	{ "이펙트2",        50,50,     atcommand_misceffect },
	{ "몹검색",         10,10,     atcommand_mobsearch },
	{ "청소",          40,40,     atcommand_cleanmap },
	{ "엔피씨대화",           20,20,     atcommand_npctalk },
	{ "펫대화",           10,10,     atcommand_pettalk },
	{ "플레이어분포",             40,40,     atcommand_users },
	{ "재분배",             40,40,     atcommand_reset },
	{ "스킬트리",         40,40,     atcommand_skilltree },
	{ "결혼",             40,40,     atcommand_marry },
	{ "이혼",           40,40,     atcommand_divorce },
	{ "소리",             40,40,     atcommand_sound },
	{ "모두변신해제",     99,99,     atcommand_undisguiseall },
	{ "모두변신",       99,99,     atcommand_disguiseall },
	{ "체인지룩",        60,60,     atcommand_changelook },
	{ "오토루팅",          10,10,     atcommand_autoloot },
	{ "자동습득",           10,10,     atcommand_autolootitem },
	{ "몹정보",            1,1,      atcommand_mobinfo },
	{ "몬스터정보",        1,1,      atcommand_mobinfo },
	{ "mi",                 1,1,      atcommand_mobinfo },
	{ "경험치",                1,1,      atcommand_exp },
	{ "입양",             40,40,     atcommand_adopt },
	{ "버전",            1,1,      atcommand_version },
	{ "영역채금",          99,99,     atcommand_mutearea },
	{ "영역침묵",              99,99,     atcommand_mutearea },
	{ "배율",              1,1,      atcommand_rates },
	{ "아이템정보",           1,1,      atcommand_iteminfo },
	{ "ii",                 1,1,      atcommand_iteminfo },
	{ "누구드롭",           1,1,      atcommand_whodrops },
	{ "몹분포",           10,10,     atcommand_whereis },
	{ "맵플래그",           99,99,     atcommand_mapflag },
	{ "대사",                20,20,     atcommand_me },
	{ "몹무시",     99,99,     atcommand_monsterignore },
	{ "비선공",      99,99,     atcommand_monsterignore },
	{ "별명",          20,20,     atcommand_fakename },
	{ "크기",              20,20,     atcommand_size },
	{ "경험치보기",           10,10,     atcommand_showexp},
	{ "제니보기",          10,10,     atcommand_showzeny},
	{ "딜레이보기",          1,1,      atcommand_showdelay},
	{ "자동거래",         10,10,     atcommand_autotrade },
	{ "at",                10,10,     atcommand_autotrade },
	{ "길마변경",          10,10,     atcommand_changegm },
	{ "파장변경",      10,10,     atcommand_changeleader },
	{ "파티설정",       10,10,     atcommand_partyoption},
	{ "초대",             1,1,      atcommand_invite },
	{ "결투",               1,1,      atcommand_duel },
	{ "중지",              1,1,      atcommand_leave },
	{ "수락",             1,1,      atcommand_accept },
	{ "거절",             1,1,      atcommand_reject },
	{ "도망",               1,1,      atcommand_away },
	{ "aw",                 1,1,      atcommand_away },
	{ "main",               1,1,      atcommand_main },
	{ "클론",             50,50,     atcommand_clone },
	{ "slaveclone",        50,50,     atcommand_clone },
	{ "evilclone",         50,50,     atcommand_clone },
	{ "tonpc",             40,40,     atcommand_tonpc },
	{ "명령어",           1,1,      atcommand_commands },
	{ "noask",              1,1,      atcommand_noask },
	{ "건의",           20,20,     atcommand_request },
	{ "호문렙업",          60,60,     atcommand_homlevel },
	{ "호문진화",      60,60,     atcommand_homevolution },
	{ "호문생성",         60,60,     atcommand_makehomun },
	{ "호문친밀",       60,60,     atcommand_homfriendly },
	{ "호문만복",         60,60,     atcommand_homhungry },
	{ "호문대화",           10,10,     atcommand_homtalk },
	{ "호문정보",            1,1,      atcommand_hominfo },
	{ "호문스텟",           1,1,      atcommand_homstats },
	{ "호문스텟변경",        60,60,     atcommand_homshuffle },
	{ "몹위치",          10,10,     atcommand_showmobs },
	{ "권성리셋",         10,10,     atcommand_feelreset },
	{ "경매",           1,1,     atcommand_auction },
	{ "메일",               1,1,      atcommand_mail },
	{ "noks",               1,1,      atcommand_ksprotection },
	{ "allowks",            40,40,      atcommand_allowks },
	{ "캐시",              60,60,     atcommand_cash },
	{ "포인트",            60,60,     atcommand_cash },
	{ "공성시작2",         60,60,    atcommand_agitstart2 },
	{ "공성종료2",           60,60,    atcommand_agitend2 },
	{ "스킬재분배",            60,60,    atcommand_resetskill },
	{ "스텟재분배",            60,60,    atcommand_resetstat },
	{ "창고목록",        40,40,    atcommand_itemlist },
	{ "카트목록",           40,40,    atcommand_itemlist },
	{ "아이템목록",           40,40,    atcommand_itemlist },
	{ "스텟",              40,40,    atcommand_stats },
	{ "아이템삭제",            60,60,    atcommand_delitem },
	{ "플레이어명령어",        1,1,     atcommand_commands },
	{ "글꼴",               1,1,     atcommand_font },
	// iLAthena 추가 명령어
	// 운영자용
	{ "버프",                99,99,     atcommand_buf },
	{ "이벤트",    99,99,     atcommand_eevent },
	{ "lockdown",		99,99,		atcommand_lockdown }, // @lockdown 명령어 [JaVb]
	{ "맵아이템",		99,99,		atcommand_itemmap },
	{ "아이템지급",			99,99,		atcommand_giveallitem },
	// 플레이어용
	{ "노점검색",			1,1,	   atcommand_searchvend },
	{ "랜파등록",		1,1,		atcommand_RandomParty_Entry },	// Party Random Entry Service [★orz。]
	{ "랜파체크",		1,1,		atcommand_RandomParty_Check },	// Party Random Entry Service [★orz。]
	{ "랜파생성",		1,1,		atcommand_RandomParty_Create },	// Party Random Entry Service [★orz。]
	{ "잡",			0,0,	   atcommand_charkami },
};
...
void do_init_atcommand()
{
	...
	// iLAthena 추가 [랜덤파티]
	add_timer_func_list(Timer_RandomParty_Invite, "Timer_RandomParty_Invite");
	return;
}
