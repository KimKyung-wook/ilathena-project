...
#ifdef WIN32
	#include <winsock2.h>
	// iLAthena 추가시작 [WPE 방어]
	#include <time.h>
	// iLAthena 추가종료 [WPE 방어]
	#include <io.h>
	// iLAthena 추가시작 [WPE 방어]
	#include <iphlpapi.h>
	// iLAthena 추가종료 [WPE 방어]
...
// iLAthena 추가시작 [다중접속]
static int multi_check     = 0;
static int multi_count     = 1;
// iLAthena 추가종료 [다중접속]
...
// iLAthena 추가시작 [다중접속]
	if(multi_check){
		...
		ip = ntohl(client_address.sin_addr.s_addr);

		for(i=0; i < fd_max; i++){
			if(!session[i] || session[i]->flag.server)
				continue;

			if(session[i]->client_addr == ip)
				count++;

			if(multi_count <= count){
				ShowWarning("'"CL_WHITE"%d.%d.%d.%d"CL_RESET"' 사용자 "CL_RED"다중접속시도"CL_RESET" 차단!\n", CONVIP(ip));
				sClose(fd);
				return -1;
			}
...
int make_connection(uint32 ip, uint16 port)
{
	...
	ShowStatus("연결중 %d.%d.%d.%d:%i\n", CONVIP(ip), port);

	result = sConnect(fd, (struct sockaddr *)(&remote_address), sizeof(struct sockaddr_in));
	if( result == SOCKET_ERROR ) {
		ShowError("연결실패 (소켓 #%d, 코드 %d)!\n", fd, sErrno);
		do_close(fd);
		return -1;
	}
...
static int connect_check(uint32 ip)
{
	...
	if( access_debug ) {
		ShowInfo("연결확인: %d.%d.%d.%d 연결%s\n", CONVIP(ip),result ? "허용." : "거부!");
	}
	return result;
}
...
static int connect_check_clear(int tid, unsigned int tick, int id, intptr data)
{
	...
	if( access_debug ){
		ShowInfo("연결확인자료 초기화: IP목록 %d 개중 %d 초기화\n", list, clear);
	}
	return list;
}
...
int socket_config_read(const char* cfgName)
{
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		// iLAthena 추가시작 [다중접속]
		else if (!strcmpi(w1,"multi_check"))
			multi_check = config_switch(w2);
		else if (!strcmpi(w1, "multi_count"))
			multi_count = atoi(w2);
		// iLAthena 추가종료 [다중접속]
#endif
...
void socket_init(void)
{
	...
	// iLAthena 추가시작 [IP권고]
	ip[0] = ((addr_[0])>>24)&0xFF;
	ip[1] = ((addr_[0])>>16)&0xFF;

	if( ip[0] == 10 ||											// 10.*.*.*
		(ip[0] == 172 && (ip[1] >= 16 && ip[1] <= 31)) ||		// 172.16.*.* ~ 172.31.*.*
		(ip[0] == 192 && ip[1] == 168) ) {						// 192.168.*
		ShowWarning(CL_BG_RED""CL_BT_WHITE"가상IP 인식, IP공유기DMZ 및 공인IP 수동설정요망.\n"CL_RESET);
	}
	// iLAthena 추가종료 [IP권고]
