 ...
 * NPC궻뼰뚼돸/뾎뚼돸
 * npc_enable
 * npc_enable_sub 뾎뚼렄궸OnTouch귽긹깛긣귩렳뛱
 *------------------------------------------*/
 ...
 * npc_parse_script->strdb_foreach궔귞뚁궽귢귡
 *------------------------------------------*/
...
/*==========================================
 * OnInit귽긹깛긣렳뛱(&렄똶귽긹깛긣둎럑)
 *------------------------------------------*/
void npc_event_do_oninit(void)
{
	ShowStatus("'"CL_WHITE"OnInit"CL_RESET"' 이벤트 라벨이 '"CL_WHITE"%d"CL_RESET"' 개의 NPC들로 부터 실행되었습니다."CL_CLL"\n", npc_event_doall("OnInit"));
...
int npc_buysellsel(struct map_session_data* sd, int id, int type)
{
	...
	if (nd->sc.option&OPTION_INVISIBLE)	// 뼰뚼돸궠귢궲궋귡
		return 1;
...
static const char* npc_parse_mapflag(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	if (!strcmpi(w3, "nosave")) {
		...
		if( state && map[m].flag.battleground )
		{
			...
			ShowWarning("맵플러그: GvG 와 전장이 %s 에 동시설정됨 ('%s' '%d'줄)\n", map[m].name, filepath, strline(buffer,start-buffer));
		}
	}
...
void npc_parsesrcfile(const char* filepath)
{
	...
	if( fp == NULL )
	{
		ShowError("npc_parsesrcfile: '%s' 파일을 찾을 수 없습니다.\n", filepath);
		return;
	}
	fseek(fp, 0, SEEK_END);
	len = ftell(fp);
...
int npc_reload(void)
{
	...
	ShowInfo ("'"CL_WHITE"%d"CL_RESET"' 개의 NPC 읽음:"CL_CLL"\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' 워프\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' 상인\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' 스크립트\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' 몬스터 소환\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' 몬스터(Cache)\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' 몬스터Not Cached)\n",
	...
	if(!CheckForCharServer()){
		ShowStatus("'"CL_WHITE"OnCharIfInit"CL_RESET"' 이벤트가 실행되는 '"CL_WHITE"%d"CL_RESET"' 개의 NPC.\n", npc_event_doall("OnCharIfInit"));
		ShowStatus("'"CL_WHITE"OnInterIfInit"CL_RESET"' 이벤트가 실행되는 '"CL_WHITE"%d"CL_RESET"' 개의 NPC.\n", npc_event_doall("OnInterIfInit"));
		ShowStatus("'"CL_WHITE"OnInterIfInitOnce"CL_RESET"' 이벤트가 실행되는 '"CL_WHITE"%d"CL_RESET"' 개의 NPC.\n", npc_event_doall("OnInterIfInitOnce"));
	}
	return 0;
}
...
int do_init_npc(void)
{
	...
	ShowStatus("NPC 읽는중...\r");
	for( file = npc_src_files; file != NULL; file = file->next )
	{
		ShowStatus("NPC 파일 읽기: %s"CL_CLL"\r", file->name);
		npc_parsesrcfile(file->name);
	}

	ShowInfo ("'"CL_WHITE"%d"CL_RESET"' 개 스크립트 읽음"CL_CLL"\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' 개의 워프\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' 개의 상점\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' 개의 스크립트\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' 소환\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' 마리의 캐시화 된 몬스터\n"
		"\t-'"CL_WHITE"%d"CL_RESET"' 마리의 캐시화 되지 않은 몬스터\n",
