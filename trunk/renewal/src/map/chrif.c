...
int chrif_setip(const char* ip)
{
	...
	ShowInfo("캐릭터 서버 IP 주소 : '"CL_WHITE"%s"CL_RESET"' -> '"CL_WHITE"%s"CL_RESET"'.\n", ip, ip2str(char_ip, ip_str));
	return 1;
}
...
int chrif_connectack(int fd)
{
	...
	if (RFIFOB(fd,2)) {
		ShowFatalError("캐릭터 서버 연결실패 %d.\n", RFIFOB(fd,2));
		exit(EXIT_FAILURE);
	}

	ShowStatus("캐릭터 서버 기록시작 성공 (연결: '"CL_WHITE"%d"CL_RESET"').\n",fd);
	chrif_state = 1;
	chrif_connected = 1;
...
int chrif_recvfamelist(int fd)
{
	...
	ShowInfo("'"CL_WHITE"%d"CL_RESET"' 개의 랭킹 캐릭터 읽음\n", total);
...
int chrif_disconnect(int fd)
{
	if(fd == char_fd) {
		char_fd = 0;
		ShowWarning("캐릭터 서버와 맵 서버 연결이 종료 되었습니다.\n\n");
		chrif_connected = 0;
...
int chrif_parse(int fd)
{
	...
	while (RFIFOREST(fd) >= 2)
	{
		...
		{
			int r = intif_parse(fd); // intif궸뱊궥

			if (r == 1) continue;	// intif궳룉뿚궢궫
			if (r == 2) return 0;	// intif궳룉뿚궢궫궕갂긢??궕뫉귟궶궋
		...
		default:
			ShowError("알수없는 패킷 (세션 #%d): 0x%x. 연결종료\n", fd, cmd);
			set_eof(fd);
			return 0;
		}
...
/*==========================================
 * timer듫릶
 * 뜞궞궻map랪궸똰궕궯궲궋귡긏깋귽귺깛긣릐릶귩char랪귉몭귡
 *------------------------------------------*/
