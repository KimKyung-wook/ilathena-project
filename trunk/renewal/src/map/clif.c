...
int clif_setip(const char* ip)
{
	...
	ShowInfo("맵 서버 IP 주소 : '"CL_WHITE"%s"CL_RESET"' -> '"CL_WHITE"%s"CL_RESET"'.\n", ip, ip2str(map_ip, ip_str));
	return 1;
}
...
void clif_setbindip(const char* ip)
{
	...
	bind_ip = host2ip(ip);
	if (bind_ip) {
		ShowInfo("맵 서버 Bind IP 주소 : '"CL_WHITE"%s"CL_RESET"' -> '"CL_WHITE"%s"CL_RESET"'.\n", ip, ip2str(bind_ip, ip_str));
	} else {
		ShowWarning("Failed to Resolve Map Server Address! (%s)\n", ip);
	}
}
...
int clif_guild_memberlist(struct map_session_data *sd)
{
	...
	for(i=0,c=0;i<g->max_member;i++){
		...
		memset(WFIFOP(fd,c*104+34),0,50);	// 긽긾갎
...
/*==========================================
 * 007d 긏깋귽귺깛긣뫀?긞긵벶귒뜛귒뒶뿹
 * map륬볺렄궸뷠뾴궶긢??귩멣궲몭귟궰궚귡
 *------------------------------------------*/
...
void clif_parse_GuildRequestInfo(int fd, struct map_session_data *sd)
{
	...
	switch( RFIFOL(fd,2) )
	{
	case 0:	// 긎깑긤딈?륃뺪갂벏뼼밎뫮륃뺪
		clif_guild_basicinfo(sd);
		clif_guild_allianceinfo(sd);
		break;
	case 1:	// 긽깛긫?깏긚긣갂뽴륟뼹깏긚긣
		clif_guild_positionnamelist(sd);
		clif_guild_memberlist(sd);
		break;
	case 2:	// 뽴륟뼹깏긚긣갂뽴륟륃뺪깏긚긣
		clif_guild_positionnamelist(sd);
...
int clif_parse(int fd)
{
	...
	if (session[fd]->flag.eof) {
		if (sd) {
			if (sd->state.autotrade) {
				...
				sd->fd = 0;
				ShowInfo("%s캐릭터 '"CL_WHITE"%s"CL_RESET"' 로그아웃 (자동거래 사용).\n", (pc_isGM(sd))?"GM ":"", sd->status.name);
			} else
			if (sd->state.active) {
				 // Player logout display [Valaris]
				ShowInfo("%s캐릭터 '"CL_WHITE"%s"CL_RESET"' 로그아웃.\n", (pc_isGM(sd))?"GM ":"", sd->status.name);
				clif_quitsave(fd, sd);
			} else {
				//Unusual logout (during log on/off/map-changer procedure)
				ShowInfo("플레이어 계정ID:%d/캐릭터ID:%d 로그아웃.\n", sd->status.account_id, sd->status.char_id);
				map_quit(sd);
			}
		} else {
			ShowInfo("'"CL_WHITE"%s"CL_RESET"' 로부터 연결 종료.\n", ip2str(session[fd]->client_addr, NULL));
		}
		do_close(fd);
		return 0;
	}
	...
	if (sd) {
		...
		{// failed to identify packet version
			ShowInfo("연결 종료: 세션 #%d 알수없는 패킷 버전 %s.\n", fd, (
				err == 1 ? "" :
...
static int packetdb_readdb(void)
{
...
#elif PACKETVER < 20071106	// 78-7b 딻뱡댥? lv99긄긲긃긏긣뾭
	...
	ShowStatus("'"CL_WHITE"packet_db.txt"CL_RESET"' 파일로부터 '"CL_WHITE"%d"CL_RESET"' 버전의 패킷을 읽었습니다.\n", clif_config.packet_db_ver);
	return 0;
}
