...
int mmo_char_fromsql(int char_id, struct mmo_charstatus* p, bool load_everything)
{
	...
	if (save_log) ShowInfo("캐릭터 읽기 요청 (%d)\n", char_id);
	...
	if (save_log) ShowInfo("캐릭터 읽음 (%d - %s): %s\n", char_id, p->name, t_msg);	//ok. all data load successfuly!
	SqlStmt_Free(stmt);
	StringBuf_Destroy(&buf);
...
int mmo_char_sql_init(void)
{
	...
	if(char_per_account == 0){
	  ShowStatus("계정 캐릭터수: '제한없음'\n");
	}else{
		ShowStatus("계정 캐릭터수: '%d'\n", char_per_account);
	}
...
int make_new_char_sql(struct char_session_data* sd, char* name_, int str, int agi, int vit, int int_, int dex, int luk, int slot, int hair_color, int hair_style)
{
	...
	ShowInfo("캐릭터 생성: 계정: %d, 캐릭터: %d, 슬롯: %d, 이름: %s\n", sd->account_id, char_id, slot, name);
	return char_id;
}
...
int mmo_char_send006b(int fd, struct char_session_data* sd)
{
	...
	if (save_log)
		ShowInfo("캐릭터 자료 읽기 ("CL_BOLD"%d"CL_RESET")\n",sd->account_id);
...
int parse_fromlogin(int fd)
{
	...
	while(RFIFOREST(fd) >= 2)
	{
		...
		case 0x2711:
			...
			if (RFIFOB(fd,2)) {
				ShowError(CL_BG_RED""CL_BT_WHITE"서버통신인증계정 설정오류\n"CL_RESET);
				ShowError(CL_BG_RED""CL_BT_WHITE"map_athena.conf char_athena.conf login테이블서 설정확인 (성별 S)\n"CL_RESET);
			}else {
...
int parse_frommap(int fd)
{
	...
	while(RFIFOREST(fd) >= 2)
	{
		...
		case 0x2afa: // Receiving map names list from the map-server
			...
			ShowStatus("맵 서버 %d 연결: %d 맵, IP %d.%d.%d.%d port %d.\n",
...
//					result = 2; // 2-gm level too low
		...
		case 0x2736: // ip address update
			...
			ShowInfo("맵 서버 IP주소 갱신 #%d to %d.%d.%d.%d.\n", id, CONVIP(server[id].ip));
			RFIFOSKIP(fd,6);
		break;
...
int lan_subnetcheck(uint32 ip)
{
	...
	if( i < subnet_count ) {
		ShowInfo("보조네트워크 검사 [%u.%u.%u.%u]: 매치 "CL_CYAN"%u.%u.%u.%u/%u.%u.%u.%u"CL_RESET"\n", CONVIP(ip), CONVIP(subnet[i].char_ip & subnet[i].mask), CONVIP(subnet[i].mask));
		return subnet[i].char_ip;
	} else {
		ShowInfo("보조네트워크 검사 [%u.%u.%u.%u]: "CL_CYAN"WAN"CL_RESET"\n", CONVIP(ip));
		return 0;
	}
}
...
int parse_char(int fd)
{
	...
	while( RFIFOREST(fd) >= 2 )
	{
				...
				return 0;
		{
			...
			ShowInfo("연결 요청 - 계정ID:%d/로그인ID1:%d/로그인ID2:%d\n", account_id, login_id1, login_id2);
			...
			if (cmd == 0x1fb) FIFOSD_CHECK(56);
		{
			...
			WFIFOHEAD(fd,46);
			ShowInfo(CL_RED"캐릭터 삭제 요청: "CL_GREEN"%d (%d)"CL_RESET"\n", sd->account_id, cid);
...
int parse_console(char* buf)
{
	...
	if( strcmpi("shutdown", command) == 0 ||
		...
		ShowInfo(CL_CYAN"콘솔: "CL_BOLD"허용."CL_RESET"\n");
	else if( strcmpi("help", command) == 0 ){
		ShowInfo(CL_BOLD"명령어 도움말:"CL_RESET"\n");
...
int check_connect_login_server(int tid, unsigned int tick, int id, intptr data)
{
	...
	ShowInfo("로그인 서버와의 연결을 시도하고 있습니다.\n");
...
int char_lan_config_read(const char *lancfgName)
{
	...
	ShowStatus("%d 개의 보조네트워크 정보를 읽음", subnet_count);
...
void sql_config_read(const char* cfgName)
{
	...
	ShowInfo("%s 파일을 읽었습니다.\n", cfgName);

	if ((fp = fopen(cfgName, "r")) == NULL) {
		ShowError("%s 파일을 찾을 수 없습니다.\n", cfgName);
		return;
	}
...
int do_init(int argc, char **argv)
{
	...
	if (strcmp(userid, "s1")==0 && strcmp(passwd, "p1")==0) {
		...
		ShowWarning(CL_BG_RED""CL_BT_WHITE"map_athena.conf char_athena.conf login테이블서 s1/p1 변경권고 (성별 S)\n"CL_RESET);
	}
	
	ShowInfo("캐릭터 서버 설정을 모두 읽었습니다.\n");

	inter_init_sql((argc > 2) ? argv[2] : inter_cfgName); // inter server 초기화
	ShowInfo("Inter 서버 설정을 모두 읽었습니다.\n");
	
	ShowInfo("캐릭터 서버를 최적화하고 있습니다.\n");
	auth_db = idb_alloc(DB_OPT_RELEASE_DATA);
	online_char_db = idb_alloc(DB_OPT_RELEASE_DATA);
	mmo_char_sql_init();
	char_read_fame_list(); //Read fame lists.
	ShowInfo("캐릭터 서버를 최적화 하였습니다.\n");
