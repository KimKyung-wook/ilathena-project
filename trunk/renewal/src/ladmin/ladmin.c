	...
	while(RFIFOREST(fd) >= 2) {
		...
		case 0x7919:	// answer of a connection request
			if (RFIFOREST(fd) < 3)
				return 0;
			if (RFIFOB(fd,2) != 0) {
				ShowMessage("로그인 오류: 비밀번호 오류, 관리 시스템 비활성화 또는 IP권한없음.\n");
				...
				ShowMessage("로그인 서버 연결성공!!\n");
				ladmin_log("Established connection.\n");
				ShowMessage("로그인 서버 버전 가져오는중...\n");
		...
		case 0x01dc:	// answer of a coding key request
				...
				return 0;
		  {
			...
			ShowMessage("MD5 키 수신.\n");
			ladmin_log("Receiving of the MD5 key.\n");
			ShowMessage("암호화된 비밀번호 전송.\n");
		...
		case 0x7531:	// Displaying of the version of the login-server
			if (RFIFOREST(fd) < 10)
				return 0;
			ShowMessage("  로그인 서버 [%s:%d]\n", loginserverip, loginserverport);
			if (((int)RFIFOB(login_fd,5)) == 0) {
				ShowMessage("  iLAthena 안정 버전-%d.%d", (int)RFIFOB(login_fd,2), (int)RFIFOB(login_fd,3));
			} else {
				ShowMessage("  iLAthena 개발 버전-%d.%d", (int)RFIFOB(login_fd,2), (int)RFIFOB(login_fd,3));
			}
			if (((int)RFIFOB(login_fd,4)) == 0)
				ShowMessage(" 버전 %d", (int)RFIFOB(login_fd,4));
		...
		case 0x7921:	// Displaying of the list of accounts
			...
			if (RFIFOW(fd,2) < 5) {
				...
				if (list_count == 0)
					...
					ShowMessage("1개의 계정 인식\n");
				else
					ShowMessage("%d개의 계정 인식\n", list_count);
				bytes_to_read = 0;
			} else {
				int i;
				WFIFOHEAD(login_fd,10);
				...
				for(i = 4; i < RFIFOW(fd,2); i += 38) {
					...
					if (list_type == 0 ||
						...
						case 4:
							...
							break;
						case 5:
							ShowMessage("%-27s\n", "블록"); // You have been blocked by the GM Team
							break;
						case 6:
							ShowMessage("%-27s\n", "exe 파일 오래됨"); // Your Game's EXE file is not the latest version
							break;
						case 7:
							ShowMessage("%-27s\n", "차단 or");
...
int ladmin_config_read(const char *cfgName)
{
	...
	fp = fopen(cfgName, "r");
	if (fp == NULL) {
		ShowMessage("\033[0m원격관리도구 설정파일 (%s) 이 없음.\n", cfgName);
		return 1;
	}

	ShowMessage("\033[0m원격관리도구 설정파일 (%s) 읽는중...\n", cfgName);
...
int do_init(int argc, char **argv)
{
	...
	ShowMessage("iLAthena 로그인 서버 원격 관리 도구\n");
	ShowMessage("(iLAthena 버전 %d.%d.%d.)\n", ATHENA_MAJOR_VERSION, ATHENA_MINOR_VERSION, ATHENA_REVISION);

	ladmin_log("Ladmin is ready.\n");
	ShowMessage("원격관리도구 \033[1;32m실행완료\033[0m.\n\n");
