...
{
	Sql* self = (Sql*)data;
	ShowInfo("SQL 서버와의 연결을 정상적으로 유지하고 있습니다.\n");
	Sql_Ping(self);
	return 0;
}
...
static void SqlStmt_P_ShowDebugTruncatedColumn(SqlStmt* self, size_t i)
{
	...
	ShowSQL("DB 오류 - data of field '%s' was truncated.\n", field->name);
	ShowDebug("column - %lu\n", (unsigned long)i);
...
int SqlStmt_NextRow(SqlStmt* self)
{
	...
	if( err == MYSQL_DATA_TRUNCATED )
	{
		...
		if( !self->bind_columns )
		{
			ShowSQL("DB 오류 - data truncated (unknown source, columns are not bound)\n");
			return SQL_ERROR;
		}
		...
		ShowSQL("DB 오류 - data truncated (unknown source)\n");
		return SQL_ERROR;
	}
#endif
	if( err )
